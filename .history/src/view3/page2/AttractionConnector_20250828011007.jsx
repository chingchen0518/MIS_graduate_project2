...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    const drawThroughLine = (svg, attractionName) => {
        ...
        attractionPaths.forEach(pathData => {
            // 找到最左邊的起點
            if (pathData.fromX < startX) {
                startX = pathData.fromX;
                startY = (pathData.fromYTop + pathData.fromYBottom) / 2;
            }
            // 找到最右邊的終點
            if (pathData.toX > endX) {
                endX = pathData.toX;
                endY = (pathData.toYTop + pathData.toYBottom) / 2;
            }
            totalPaths++;
        });
