...
const ScheduleShow = (props) => {
    ...
    // 檢測排序變化並觸發動畫
    useEffect(() => {
        // 如果是第一次載入或沒有之前的順序，不執行動畫
        if (!previousOrder || previousOrder.length === 0) {
            return;
        }

        // 檢查是否有順序變化（通過比較相鄰的行程ID）
        const prevElement = scheduleRef.current?.previousElementSibling;
        const nextElement = scheduleRef.current?.nextElementSibling;
        
        const currentPrevId = prevElement?.getAttribute('data-schedule-id');
        const currentNextId = nextElement?.getAttribute('data-schedule-id');
        
        const prevPrevId = previousOrder.prevId;
        const prevNextId = previousOrder.nextId;

        // 如果相鄰元素發生變化，表示順序改變了
        if (currentPrevId !== prevPrevId || currentNextId !== prevNextId) {
            startSortingAnimation();
        }
    }, [scheduleItems, previousOrder]);
