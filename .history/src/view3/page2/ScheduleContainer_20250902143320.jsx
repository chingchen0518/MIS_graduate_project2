...
const ScheduleContainer = ({ t_id, usedAttractions = [], onAttractionUsed, onShowRoute, onHideRoute, onAttractionSelect, filterConditions }) => {
    ...
    const [isReordering, setIsReordering] = useState(false); // 新增：控制是否正在重新排序

    const timeColumnRef = useRef(null);
    const scheduleListRef = useRef(null); // 新增：用於AttractionConnector的ref
    const sortTimeoutRef = useRef(null); // 新增：用於延遲排序的timeout
    ...
    const handleSortChange = (event) => {
        ...
        setIsReordering(true); // 標記開始重新排序
        
        // 設定一個timeout來重置reordering狀態
    ...
    useEffect(() => {
        const applySorting = async () => {
            if (schedules.length > 0) {
                ...
                // 使用 requestAnimationFrame 來確保DOM更新的時機
    ...
    return (
        <div className="schedule_container">
            ...
            <div className={`schedule_list attraction-connector-container ${isReordering ? 'reordering' : ''}`} ref={scheduleListRef} style={{ position: 'relative' }}>
                    ...
                    <>
                        {schedules.map((schedule, index) => (
                            <ScheduleShow
                                ...
                                isReordering={isReordering} // 傳遞重新排序狀態
                                sortIndex={index} // 傳遞當前排序索引
                            />
                        ))}
