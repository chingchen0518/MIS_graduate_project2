...
// 新增API：計算特定行程的總預算 - 依日期篩選Schedule後查詢預算
app.get('/api/schedule_budget/:t_id/:s_id/:date', (req, res) => {
  const { t_id, s_id, date } = req.params;

  console.log(`🔍 查詢預算 - t_id: ${t_id}, s_id: ${s_id}, date: ${date}`);

  const query = `SELECT SUM(a.budget) as total_budget 
                 FROM Schedule s
                 JOIN Schedule_include si ON s.s_id = si.s_id AND s.t_id = si.t_id
                 JOIN Attraction a ON si.a_id = a.a_id
                 WHERE s.t_id = ? AND s.s_id = ? AND s.date = ?`;
  const values = [t_id, s_id, date];

  console.log('💰 執行預算查詢 SQL:', query);
  console.log('💰 查詢參數:', values);

  connection.query(query, values, (err, results) => {
    if (err) {
      console.error('❌ 預算計算錯誤:', err);
      res.status(200).json({ total_budget: 0 });
    } else {
      const totalBudget = results[0]?.total_budget || 0;
      console.log(`✅ Schedule ${s_id} 在日期 ${date} 的預算: ${totalBudget}`);
      res.status(200).json({ total_budget: totalBudget });
    }
  });
});

// 新增API：獲取行程投票狀態 - 依日期篩選Schedule後查詢投票
app.get('/api/schedule_votes/:t_id/:s_id/:date', (req, res) => {
  const { t_id, s_id, date } = req.params;

  console.log(`🗳️ 查詢投票 - t_id: ${t_id}, s_id: ${s_id}, date: ${date}`);

  // 獲取該行程的所有投票統計，連接Schedule和Evaluate表
  const query = `SELECT 
                   COUNT(CASE WHEN e.good = true THEN 1 END) as total_likes,
                   COUNT(CASE WHEN e.bad = true THEN 1 END) as total_dislikes
                 FROM Schedule s
                 JOIN Evaluate e ON s.s_id = e.s_id AND s.t_id = e.t_id
                 WHERE s.t_id = ? AND s.s_id = ? AND s.date = ?`;

  console.log('🗳️ 執行投票查詢 SQL:', query);
  console.log('🗳️ 查詢參數:', [t_id, s_id, date]);

  connection.query(query, [t_id, s_id, date], (err, results) => {
    if (err) {
      console.error('❌ 投票查詢錯誤:', err);
      ...
      console.log(`✅ Schedule ${s_id} 在日期 ${date} 的投票: 讚 ${votes.total_likes}, 倒讚 ${votes.total_dislikes}`);
      res.status(200).json(votes);
    }
  });
});

// 新增API：投票給行程 - 依日期篩選Schedule後進行投票
...
app.get('/api/view3_trip_budget_range/:t_id', (req, res) => {
  ...
  // 先獲取最小預算
  const minQuery = `
    SELECT MIN(a.budget) as min_budget
    FROM Attraction a
    INNER JOIN Schedule_include si ON a.a_id = si.a_id
    INNER JOIN Schedule s ON si.s_id = s.s_id
