...
const ScheduleShow = (props) => {
    ...
    const handleVote = async (voteType) => {
        try {
            // 為了測試多用戶投票，每次使用隨機用戶ID (1-5)
            // 實際應用中應該從用戶登錄狀態獲取真實的用戶ID
            const currentUserId = Math.floor(Math.random() * 5) + 1; // 隨機生成1-5的用戶ID
            const formattedDate = formatDate(props.date);

            console.log(`User ${currentUserId} voting ${voteType} for schedule t_id:${props.t_id}, s_id:${props.s_id}`);
