...
// API: 取得指定trip的景點預算範圍（修改為按Schedule計算最大值）
app.get('/api/view3_trip_budget_range/:t_id', (req, res) => {
  ...
  const query = `
    ...
    FROM (
      SELECT s.s_id, SUM(a.budget) as total_budget
      FROM attraction a
...
// API: 取得參與指定trip的使用者清單
app.get('/api/trip_users/:t_id', (req, res) => {
  const { t_id } = req.params;

  const query = `
    SELECT DISTINCT u.u_id, u.u_name, u.u_img, j.color
    FROM User u
  ...
  connection.query(query, [t_id], (err, results) => {
    if (err) {
      console.error('❌ 取得trip使用者時出錯：', err.message);
      res.status(500).json({ error: err.message });
      return;
    }
