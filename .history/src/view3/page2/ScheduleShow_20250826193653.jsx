...
const ScheduleShow = (props) => {
    ...
    const [tripMembers, setTripMembers] = useState([]); // 新增：行程成員
    const [currentUserId] = useState(1); // 當前用戶ID，實際應該從props或context獲取
    ...
    useEffect(() => {
        const fetchTripMembers = async () => {
            try {
                ...
                const response = await fetch(`http://localhost:3001/api/trip_users/${props.t_id}`);
                if (response.ok) {
                    const data = await response.json();
                    console.log('Trip members data:', data);
                    setTripMembers(data.users || []);
                } else {
    ...
    const renderVoteRing = (voteType, voteCount) => {
        const totalMembers = tripMembers.length || 5; // 如果沒有成員資料，預設5人
        const radius = 10; // 縮小光圈半徑
        const centerX = 15; // 調整中心點X
        const centerY = 15; // 調整中心點Y
        
        // 找到當前用戶在成員列表中的位置
        const currentUserIndex = tripMembers.findIndex(member => member.u_id === currentUserId);
        
        // 生成所有成員的點，數量等於總人數
        const dashArray = [];
        for (let i = 0; i < totalMembers; i++) {
            // 調整角度計算，確保當前用戶在12點鐘方向
            let adjustedIndex = i;
            if (currentUserIndex !== -1) {
                adjustedIndex = (i - currentUserIndex + totalMembers) % totalMembers;
            }
            
            const angle = (adjustedIndex * 360) / totalMembers - 90; // 從12點鐘方向開始，順時針分布
            const isLit = i < voteCount; // 是否點亮（前voteCount個點）
            const isCurrentUser = currentUserIndex !== -1 && i === currentUserIndex; // 當前用戶位置
                    ...
                    {dashArray.map((dash, index) => {
                        ...
                        return (
                            <circle
                                ...
                                strokeWidth={dash.isCurrentUser ? '0.5' : '0'}
                            />
                        );
                    })}
                </svg>
                ...
                <div className="user_info">
                    ...
                    <div className="vote_buttons">
                        <div 
                            ...
                            title="讚"
                        >
                            {renderVoteRing('like', voteData.total_likes)}
                        </div>
                        <div
                            ...
                            title="倒讚"
                        >
                            {renderVoteRing('dislike', voteData.total_dislikes)}
                        </div>
                    </div>
                </div>
