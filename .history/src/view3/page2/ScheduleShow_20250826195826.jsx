...
const ScheduleShow = (props) => {
    ...
    const [voteData, setVoteData] = useState({ total_likes: 0, total_dislikes: 0, user_vote: null }); // 新增：投票數據和用戶投票狀態
    ...
    useEffect(() => {
        const fetchVotes = async () => {
            try {
                const currentUserId = 1; // 暫時使用固定值
                const formattedDate = formatDate(props.date);
                console.log(`Fetching votes for t_id: ${props.t_id}, s_id: ${props.s_id}, date: ${formattedDate}`);
                const response = await fetch(`http://localhost:3001/api/schedule_votes/${props.t_id}/${props.s_id}/${formattedDate}?userId=${currentUserId}`);
    ...
    const handleVote = async (voteType) => {
        try {
            ...
            // 如果點擊的是相同類型的投票，則取消投票
            const finalVoteType = voteData.user_vote === voteType ? null : voteType;
            ...
            if (response.ok) {
                // 重新獲取投票數據
                const voteResponse = await fetch(`http://localhost:3001/api/schedule_votes/${props.t_id}/${props.s_id}/${formattedDate}?userId=${currentUserId}`);
                if (voteResponse.ok) {
    ...
    // 根據總票數計算顏色深淺
    const getColorIntensity = (baseColor, totalVotes) => {
        // 設定最小和最大不透明度
        const minOpacity = 0.3;
        const maxOpacity = 1.0;
        
        // 設定票數範圍（可以根據實際需求調整）
        const maxVotesForFullIntensity = 20;
        
        // 計算不透明度
        const opacity = minOpacity + (Math.min(totalVotes, maxVotesForFullIntensity) / maxVotesForFullIntensity) * (maxOpacity - minOpacity);
        
        // 根據基礎顏色和不透明度返回顏色
        if (baseColor === 'like') {
            return `rgba(76, 175, 80, ${opacity})`; // 綠色的不同深淺
        } else {
            return `rgba(244, 67, 54, ${opacity})`; // 紅色的不同深淺
        }
    };
                        ...
                        <div className="vote_bar_container">
                            <div
                                ...
                                style={{
                                    ...
                                    backgroundColor: getColorIntensity('like', voteData.total_likes + voteData.total_dislikes),
                                    height: '30px',
                                    display: 'flex',
                                    alignItems: 'center',
                                    ...
                                    border: voteData.user_vote === 'like' ? '2px solid #2e7d32' : 'none',
                                    boxShadow: voteData.user_vote === 'like' ? '0 0 5px rgba(46, 125, 50, 0.5)' : 'none'
                                }}
                                onClick={() => handleVote('like')}
                                title={`讚 (${voteData.total_likes})${voteData.user_vote === 'like' ? ' - 已投票' : ''}`}
                            >
                                👍
                            </div>
                            <div
                                ...
                                style={{
                                    ...
                                    backgroundColor: getColorIntensity('dislike', voteData.total_likes + voteData.total_dislikes),
                                    height: '30px',
                                    display: 'flex',
                                    alignItems: 'center',
                                    justifyContent: 'center',
                                    ...
                                    border: voteData.user_vote === 'dislike' ? '2px solid #c62828' : 'none',
                                    boxShadow: voteData.user_vote === 'dislike' ? '0 0 5px rgba(198, 40, 40, 0.5)' : 'none'
                                }}
                                onClick={() => handleVote('dislike')}
                                title={`倒讚 (${voteData.total_dislikes})${voteData.user_vote === 'dislike' ? ' - 已投票' : ''}`}
                            >
                                👎
                            </div>
                        </div>
