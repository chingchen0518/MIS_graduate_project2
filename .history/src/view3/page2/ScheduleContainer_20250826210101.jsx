...
const ScheduleContainer = ({ t_id, usedAttractions = [], onAttractionUsed, filterConditions }) => {
    ...
    const calculateScheduleStats = async (schedule) => {
        try {
            const response = await fetch(`http://localhost:3001/api/schedule_stats/${t_id}/${schedule.s_id}`);
            if (response.ok) {
                const stats = await response.json();
                
                console.log(`📊 統計資料 - 行程 ${schedule.s_id}:`, stats);
