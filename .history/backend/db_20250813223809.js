...
// 獲取行程日期 API
app.get('/api/trip-dates/:t_id', (req, res) => {
  const { t_id } = req.params;
  
  // 查詢該行程的所有日期
  const sql = 'SELECT DISTINCT date FROM Schedule WHERE t_id = ? ORDER BY date ASC';
  
  connection.query(sql, [t_id], (err, results) => {
    if (err) {
      console.error('❌ 查詢行程日期時出錯：', err.message);
      return res.status(500).json({ error: err.message });
    }
    ...
    const dates = results.map(row => ({
      date: row.date,
      displayText: new Date(row.date).toLocaleDateString('zh-TW', { 
        month: '2-digit', 
        day: '2-digit' 
      })
    }));
