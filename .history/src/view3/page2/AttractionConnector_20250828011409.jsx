...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    const attractionGroupsRef = useRef({});
    ...
    const drawConnections = () => {
        ...
        svg
            ...
            .style('pointer-events', 'none') // SVG本身不接收事件
            .style('z-index', '1000')
                ...
                if (next.scheduleIndex - current.scheduleIndex === 1) {
                    ...
                    if (isConnectionVisible(current, next)) {
                        ...
                        paths.forEach(path => {
                            ...
                            path.centerLine = calculateCenterLine(path); // 計算中心線
        ...
        // 儲存分組資訊供懸停使用
        attractionGroupsRef.current = attractionGroups;
    ...
    // 計算面積中心線的路徑
    const calculateCenterLine = (pathData) => {
        const { fromX, fromYTop, fromYBottom, toX, toYTop, toYBottom } = pathData;
        
        // 計算起點和終點的中心位置
        const fromCenterY = (fromYTop + fromYBottom) / 2;
        const toCenterY = (toYTop + toYBottom) / 2;
        
        // 創建一條直線路徑
        const centerPath = d3.path();
        centerPath.moveTo(fromX, fromCenterY);
        centerPath.lineTo(toX, toCenterY);
        
        return {
            path: centerPath.toString(),
            fromX,
            fromCenterY,
            toX,
            toCenterY
    ...
    const drawConnectionPath = (svg, pathData, hasIntersection) => {
        ...
        // 繪製主要連接面
        const connectionPath = targetGroup.append('path')
            .attr('d', pathData.path)
            .attr('class', `attraction-connection attraction-${pathData.attractionName.replace(/\s+/g, '-')}`)
            ...
            .style('pointer-events', 'all') // 允許滑鼠事件
            .style('cursor', 'pointer');

        // 添加懸停事件
        connectionPath
            .on('mouseenter', function(event) {
                console.log('懸停在連接面上:', pathData.attractionName);
                setHoveredGroup(pathData.attractionName);
                drawCenterLines(svg, pathData.attractionName);
            })
            .on('mouseleave', function(event) {
                console.log('離開連接面:', pathData.attractionName);
                setHoveredGroup(null);
                removeCenterLines(svg);
            });
    };

    // 繪製中心線
    const drawCenterLines = (svg, attractionName) => {
        const targetGroup = svg.mainGroup || svg;
        const attractionPaths = attractionGroupsRef.current[attractionName] || [];

        // 移除之前的中心線
        targetGroup.selectAll('.center-line').remove();

        // 為該景點的所有連接繪製中心線
        attractionPaths.forEach((pathData, index) => {
            if (pathData.centerLine) {
                targetGroup.append('path')
                    .attr('d', pathData.centerLine.path)
                    .attr('class', 'center-line')
                    .style('stroke', '#FFD700') // 金色粗線
                    .style('stroke-width', '4px')
                    .style('stroke-opacity', '0.9')
                    .style('fill', 'none')
                    .style('stroke-linecap', 'round')
                    .style('pointer-events', 'none')
                    .style('filter', 'drop-shadow(0 0 3px rgba(255, 215, 0, 0.6))'); // 添加發光效果
            }
        });
    };

    // 移除中心線
    const removeCenterLines = (svg) => {
        const targetGroup = svg.mainGroup || svg;
        targetGroup.selectAll('.center-line').remove();
    };
