...
app.get('/api/schedule_votes/:t_id/:s_id/:date', (req, res) => {
  ...
  console.log(`查詢行程投票統計: t_id=${t_id}, s_id=${s_id}, date=${date}`);
  ...
  connection.query(validateQuery, [t_id, s_id, date], (validateErr, scheduleExists) => {
    if (validateErr) {
      ...
      res.status(500).json({ total_likes: 0, total_dislikes: 0 });
      return;
    }

    if (scheduleExists.length === 0) {
      console.log(`指定日期 ${date} 沒有找到行程 s_id=${s_id}`);
    ...
    connection.query(query, [t_id, s_id], (err, results) => {
      if (err) {
        ...
        console.log(`投票統計結果: likes=${votes.total_likes}, dislikes=${votes.total_dislikes}`);
...
app.get('/api/user_vote/:t_id/:s_id/:u_id', (req, res) => {
  ...
  console.log(`查詢用戶投票狀態: t_id=${t_id}, s_id=${s_id}, u_id=${u_id}`);
  ...
  connection.query(query, [t_id, s_id, u_id], (err, results) => {
    ...
    if (results.length === 0) {
      // 用戶沒有投票
      console.log(`用戶 ${u_id} 對行程 ${s_id} 沒有投票記錄`);
      ...
      console.log(`用戶 ${u_id} 對行程 ${s_id} 的投票狀態: ${vote_type}`);
...
app.post('/api/schedule_vote/:t_id/:s_id/:u_id/:date', (req, res) => {
  ...
  connection.query(validateQuery, [t_id, s_id, date], (validateErr, scheduleExists) => {
    ...
    connection.query(checkQuery, [u_id, s_id, t_id], (err, existing) => {
      ...
      if (existing.length > 0) {
        // 如果重複點擊相同的投票（這個邏輯現在由前端處理，後端不會收到相同的 vote_type）
        // 直接更新現有投票
