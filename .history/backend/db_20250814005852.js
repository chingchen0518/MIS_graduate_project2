...
app.get('/api/schedule_budget/:t_id/:s_id/:date', (req, res) => {
  ...
  // 先依日期篩選Schedule，然後通過s_id查詢Schedule_include和Attraction
  const query = `
  ...
  console.log(`Calculating budget for t_id:${t_id}, s_id:${s_id}, date:${date}`);
  ...
  connection.query(query, values, (err, results) => {
    if (err) {
      ...
      console.log(`Budget calculation result: ${totalBudget}`);
