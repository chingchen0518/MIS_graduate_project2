...
const Filter = ({ t_id, onCategoryChange }) => {
    ...
    const [attractionCounts, setAttractionCounts] = useState({});
                ...
                if (categoriesData.success) {
                    ...
                    try {
                        ...
                        categoriesData.categories.forEach(category => {
                            ...
                            counts[category] = categoryAttractions.length;
                            categoryDetails[category] = categoryAttractions;
                        });
                        
                        setAttractionCounts(counts);
                        setCategoryAttractions(categoryDetails);
                    } catch (attractionError) {
                        console.error('Error fetching attractions:', attractionError);
                        // 設置預設計數
                        const defaultCounts = {};
                        categoriesData.categories.forEach(cat => {
                            defaultCounts[cat] = Math.floor(Math.random() * 10) + 1;
                        });
                        setAttractionCounts(defaultCounts);
                    }
                } else {
                    console.error('Failed to fetch categories:', categoriesData.error);
                    // 設置預設類別以供測試
                    const defaultCategories = ['文化', '自然', '餐廳', '娛樂', '購物', '住宿'];
