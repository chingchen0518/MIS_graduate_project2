...
const ScheduleShow = (props) => {
    ...
    const handleVote = async (voteType) => {
        try {
            ...
            // 添加點擊動畫效果
            const button = document.querySelector(`.${voteType}_button`);
            if (button) {
                button.classList.add('clicked');
                createParticleEffect(button, voteType);
            ...
            if (response.ok) {
                ...
                if (voteResponse.ok) {
                    ...
                    setVoteData(prevData => {
                        // 添加數字更新動畫
                        setTimeout(() => {
                            const counts = document.querySelectorAll('.vote_count');
                            counts.forEach(count => {
                                count.classList.add('updated');
                                setTimeout(() => count.classList.remove('updated'), 600);
                            });
                        }, 100);
                        return voteData;
                    });
    ...
    // 獲取投票數字的特殊樣式類別
    const getVoteCountClass = (count) => {
        let classes = 'vote_count';
        if (count > 20) {
            classes += ' rainbow';
        }
        if (count >= 10) {
    ...
    // 創建粒子效果
    const createParticleEffect = (button, voteType) => {
        const particlesContainer = button.querySelector('.particles') || document.createElement('div');
        ...
        for (let i = 0; i < 12; i++) {
            ...
            const colors = voteType === 'like' ? 
                ['#4ade80', '#22c55e', '#16a34a', '#ffffff'] : 
                ['#ef4444', '#dc2626', '#b91c1c', '#ffffff'];
                ...
                <div className="user_info">
                    ...
                    <div className="vote_buttons">
                        ...
                        >
                            ...
                            <span className={getVoteCountClass(voteData.total_likes)}>{voteData.total_likes}</span>
                        </button>
                        <button
                        ...
                        >
                            <span className="vote_icon">👎</span>
                            <span className={getVoteCountClass(voteData.total_dislikes)}>{voteData.total_dislikes}</span>
                        </button>
                    </div>
                </div>
