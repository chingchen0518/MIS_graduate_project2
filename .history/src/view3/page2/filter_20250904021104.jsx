...
const Filter = ({ t_id, onCategoryChange, onFilterChange, onAttractionSelect }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // 獲取指定trip的景點類別
                const categoriesResponse = await fetch(`http://localhost:3001/api/attraction_categories/${actualTripId}`);
                const categoriesData = await categoriesResponse.json();

                // 獲取該trip的預算範圍
                const budgetResponse = await fetch(`http://localhost:3001/api/view3_trip_budget_range/${actualTripId}`);
                ...
                // 獲取該trip的參與使用者
                try {
                    const usersResponse = await fetch(`http://localhost:3001/api/trip_users/${actualTripId}`);
    ...
    }, [selectedCategories]); // 移除 onCategoryChange 依賴項，因為它現在是穩定的 useCallback
    ...
    }, [costRange, selectedAttractions, selectedUsers]); // 移除 onFilterChange 依賴項，因為它現在是穩定的 useCallback
    ...
    if (loading) {
        return <div className="filter-container loading">Loading...</div>;
    }

    return (
        <div className="filter-container">
            <div className="filter-header" onClick={(e) => e.stopPropagation()}>
                <span className="filter-icon" onClick={handleFilterIconClick}>🔍</span>
                <span className="filter-title">Filter:</span>

                {/* 費用範圍滑桿 */}
                <div className="cost-range-container" onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className="expanded-category">
                                ...
                                <div className="attractions-grid">
                                    {(categoryAttractions[expandedCategory] || []).map((attraction) => (
                                        ...
                                        >
                                            <label className="attraction-checkbox" onClick={(e) => e.stopPropagation()}>
                                                <input
                                                    type="checkbox"
