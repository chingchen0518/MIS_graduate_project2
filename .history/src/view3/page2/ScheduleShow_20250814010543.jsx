...
const ScheduleShow = (props) => {
    ...
    const handleVote = async (voteType) => {
        try {
            ...
            console.log(`📝 前端提交投票 - t_id: ${props.t_id}, s_id: ${props.s_id}, u_id: ${currentUserId}, date: ${formattedDate}, vote_type: ${voteType}`);
            ...
            console.log('📝 投票提交回應狀態:', response.status);

            if (response.ok) {
                console.log('✅ 投票提交成功，重新獲取投票數據');
                // 重新獲取投票數據
                const voteResponse = await fetch(`http://localhost:3001/api/schedule_votes/${props.t_id}/${props.s_id}/${formattedDate}`);
                if (voteResponse.ok) {
                    const voteData = await voteResponse.json();
                    setVoteData(voteData);
                    console.log('✅ 投票數據更新完成:', voteData);
                }
            } else {
                console.error('❌ 投票提交失敗，狀態:', response.status);
                const errorText = await response.text();
                console.error('❌ 投票提交錯誤:', errorText);
            }
        } catch (error) {
            console.error('❌ 投票提交網路錯誤:', error);
        }
    };
