...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    const [highlightedGroup, setHighlightedGroup] = useState(null); // 高亮的景點組
    const [isInteracting, setIsInteracting] = useState(false); // 是否正在交互中
    ...
    const drawConnections = () => {
        ...
        svg
            ...
            .style('pointer-events', 'auto') // 改為auto以允許交互
            .style('z-index', '1000')
        ...
        allPaths.forEach((pathData, index) => {
            ...
            drawConnectionPath(svg, pathData, hasIntersection, attractionGroups);
        });

        // 添加交互事件監聽
        setupInteractionEvents(svg, attractionGroups);
    ...
    const drawConnectionPath = (svg, pathData, hasIntersection, attractionGroups) => {
        const baseColor = hasIntersection ? '#FF4444' : '#2196F3'; // 紅色如果相交，否則藍色
        
        // 判斷當前是否應該高亮
        const isHighlighted = !isInteracting || highlightedGroup === pathData.attractionName;
        const isDimmed = isInteracting && highlightedGroup !== pathData.attractionName;
        
        // 根據交互狀態調整顏色和透明度
        let color = baseColor;
        let fillOpacity = isHighlighted ? 0.5 : (isDimmed ? 0.1 : 0.3);
        let strokeOpacity = isHighlighted ? 1 : (isDimmed ? 0.3 : 0.8);
        let strokeWidth = isHighlighted ? '2px' : '1px';
        ...
        const path = targetGroup.append('path')
            ...
            .attr('data-attraction-group', pathData.attractionName) // 添加數據屬性用於查找
            .style('fill', color)
            ...
            .style('transition', 'all 0.3s ease'); // 添加過渡效果
    ...
    // 設置交互事件
    const setupInteractionEvents = (svg, attractionGroups) => {
        const targetGroup = svg.mainGroup || svg;
        
        // 為每個連線路徑添加事件監聽
        targetGroup.selectAll('.attraction-connection')
            .style('pointer-events', 'auto') // 啟用鼠標事件
            .on('mouseenter', function(event) {
                const attractionName = d3.select(this).attr('data-attraction-group');
                handleLineHover(attractionName, true);
            })
            .on('mouseleave', function(event) {
                const attractionName = d3.select(this).attr('data-attraction-group');
                handleLineHover(attractionName, false);
            })
            .on('click', function(event) {
                event.stopPropagation();
                const attractionName = d3.select(this).attr('data-attraction-group');
                handleLineClick(attractionName);
            });
    ...
    // 處理線條懸停
    const handleLineHover = (attractionName, isEntering) => {
        if (!isInteracting) { // 只有在沒有點擊鎖定時才響應懸停
            if (isEntering) {
                setHighlightedGroup(attractionName);
                setIsInteracting(true);
                updateConnectionStyles(attractionName);
            } else {
                // 添加短暫延遲，避免鼠標快速移動時閃爍
                setTimeout(() => {
                    setHighlightedGroup(null);
                    setIsInteracting(false);
                    updateConnectionStyles(null);
                }, 100);
            }
        }
    };
    ...
    // 處理線條點擊
    const handleLineClick = (attractionName) => {
        if (highlightedGroup === attractionName && isInteracting) {
            // 如果點擊的是已經高亮的組，則取消高亮
            clearHighlight();
        } else {
            // 否則高亮該組
    ...
    // 更新連線樣式
    const updateConnectionStyles = (highlightedAttraction) => {
        const svg = d3.select(svgRef.current);
        const targetGroup = svg.select('g'); // 主要群組
        
        targetGroup.selectAll('.attraction-connection')
            .transition()
            .duration(300)
            .style('fill-opacity', function() {
                const groupName = d3.select(this).attr('data-attraction-group');
                if (!highlightedAttraction) return 0.3; // 默認透明度
                return groupName === highlightedAttraction ? 0.6 : 0.1; // 高亮或暗淡
            })
            .style('stroke-opacity', function() {
                const groupName = d3.select(this).attr('data-attraction-group');
                if (!highlightedAttraction) return 0.8; // 默認透明度
                return groupName === highlightedAttraction ? 1 : 0.2; // 高亮或暗淡
            })
            .style('stroke-width', function() {
                const groupName = d3.select(this).attr('data-attraction-group');
                if (!highlightedAttraction) return '1px'; // 默認寬度
                return groupName === highlightedAttraction ? '3px' : '1px'; // 高亮時加粗
            })
            .style('filter', function() {
                const groupName = d3.select(this).attr('data-attraction-group');
                if (!highlightedAttraction) return 'none'; // 默認無濾鏡
                if (groupName === highlightedAttraction) {
                    // 高亮時添加發光效果
                    const color = d3.select(this).style('stroke');
                    return `drop-shadow(0 0 12px ${color}) brightness(1.2)`;
                }
                return 'brightness(0.5)'; // 暗淡效果
            });
    };
    ...
    return (
        <svg
            ...
            style={{
                ...
                width: '100vw',
                height: '100vh',
                pointerEvents: 'auto', // 改為auto以支持交互
                zIndex: 1000,
                overflow: 'visible'
            }}
        />
