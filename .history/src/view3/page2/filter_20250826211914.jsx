...
const Filter = ({ t_id, selectedDate, onCategoryChange, onFilterChange }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // 獲取該trip和日期的預算範圍
                let budgetApi = `http://localhost:3001/api/view3_trip_budget_range/${t_id}`;
                if (selectedDate) {
                    budgetApi += `?date=${encodeURIComponent(selectedDate)}`;
                }
                const budgetResponse = await fetch(budgetApi);
                ...
                if (budgetData.success) {
                    const min = budgetData.minBudget || 0;
                    const max = budgetData.maxBudget || 1000;
                    // 確保最大值至少比最小值大1
                    const adjustedMax = max > min ? max : min + 100;
                    
                    setMinBudget(min);
                    setMaxBudget(adjustedMax);
                    setCostRange([min, adjustedMax]);
                } else {
    ...
    const getSliderBackground = () => {
        ...
        if (range <= 0) return '#ddd';
        
        const percentage = Math.max(0, Math.min(100, ((costRange[1] - minBudget) / range) * 100));
