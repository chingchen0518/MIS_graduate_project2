...
app.get('/api/attraction_categories', (req, res) => {
  const sql = 'SELECT DISTINCT category FROM Attraction WHERE category IS NOT NULL AND category != ""';
  
  connection.query(sql, (err, rows) => {
    if (err) {
      console.error('❌ 查詢類別時出錯：', err.message);
      return res.status(500).json({ error: err.message });
    }
    
    // 提取類別名稱
    const categories = rows.map(row => row.category);
    console.log('✅ 查詢到景點類別:', categories);
    res.json(categories);
  });
});

// 獲取所有城市
app.get('/api/attraction_cities', (req, res) => {
  const sql = 'SELECT DISTINCT city FROM Attraction WHERE city IS NOT NULL AND city != ""';
  
  connection.query(sql, (err, rows) => {
    if (err) {
      console.error('❌ 查詢城市時出錯：', err.message);
      return res.status(500).json({ error: err.message });
    }
    
    // 提取城市名稱
    const cities = rows.map(row => row.city);
    console.log('✅ 查詢到城市:', cities);
    res.json(cities);
  });
});
