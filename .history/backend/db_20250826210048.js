...
// 新增API：獲取行程統計資料
app.get('/api/schedule_stats/:t_id/:s_id', (req, res) => {
  const { t_id, s_id } = req.params;

  const query = `SELECT 
                   COUNT(*) as attractions_count,
                   SUM(s.height) as total_duration,
  ...
  connection.query(query, values, (err, results) => {
    if (err) {
      ...
      // 處理categories字串
      const categoriesArray = stats.categories ? stats.categories.split(',') : [];
      
      res.status(200).json({
        attractionsCount: stats.attractions_count || 0,
