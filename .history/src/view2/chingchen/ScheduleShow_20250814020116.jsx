...
const ScheduleShow = (props) => {
    ...
    const getScheduleOpacity = () => {
        if (!props.filterConditions) return 1;
        
        const { budgetRange } = props.filterConditions;
        if (!budgetRange) return 1;
        
        // 這裡假設預算是350，實際應該從API獲取
        const scheduleBudget = 350;
        const [minBudget, maxBudget] = budgetRange;
        
        if (scheduleBudget >= minBudget && scheduleBudget <= maxBudget) {
            return 1; // 在範圍內，完全不透明
        } else {
            return 0.3; // 在範圍外，變透明
        }
    ...
    // 檢查景點是否被選中
    const isAttractionSelected = (attractionName) => {
        if (!props.filterConditions || !props.filterConditions.selectedAttractions) return false;
        return props.filterConditions.selectedAttractions.includes(attractionName);
    };
