...
const ScheduleShow = (props) => {
    ...
    // 檢查是否有景點被選中
    const hasSelectedAttractions = scheduleItems.some(item => 
        selectedAttractions.includes(item.a_id)
    );
    ...
    const getScheduleBrightness = () => {
        if (hasSelectedAttractions) return 'brightness(1.2)'; // 有選中景點時發亮
        return 'brightness(1)';
    };
