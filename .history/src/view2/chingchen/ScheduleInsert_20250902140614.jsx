...
const ScheduleInsert = ({
    ...
    const [isReordering, setIsReordering] = useState(false); // 新增：重新排序動畫狀態
    ...
    const getChildData = (func_height, func_x, func_y,func_a_id) => {
        ...
        setAttractions(prev => {
            ...
            const newOrdered = updated.map(item => ({ ...item, sequence: seqMap.get(item.a_id) }));
            
            // 檢查是否有序列變化
            const hasSequenceChange = prev.some((oldItem) => {
                const newItem = newOrdered.find(item => item.a_id === oldItem.a_id);
                return newItem && oldItem.sequence !== newItem.sequence;
            });
