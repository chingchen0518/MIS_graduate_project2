...
// API: 取得指定Schedule的所有景點預算總和
app.get('/api/schedule_budget/:s_id', (req, res) => {
  ...
  const query = `
    ...
    FROM Schedule s
    LEFT JOIN Schedule_include si ON s.s_id = si.s_id
    LEFT JOIN Attraction a ON si.a_id = a.a_id
    WHERE s.s_id = ?
    GROUP BY s.s_id, s.title
  `;

  connection.query(query, [s_id], (err, results) => {
    if (err) {
      console.error('❌ 取得Schedule預算失敗:', err);
      return res.status(500).json({ 
        success: false, 
        message: '取得Schedule預算失敗',
        error: err.message 
      });
    }
    ...
    if (results.length === 0) {
      return res.status(404).json({ 
        success: false, 
        message: '找不到指定的Schedule' 
      });
    }

    console.log('✅ Schedule預算查詢成功:', results[0]);
    res.json({
      success: true,
      data: results[0]
    });
...
// API: 新增或更新Schedule的評價（讚/倒讚）
app.post('/api/schedule_vote', (req, res) => {
  const { u_id, s_id, t_id, vote_type } = req.body;

  // 檢查參數
  if (!u_id || !s_id || !t_id || !vote_type) {
    return res.status(400).json({
      success: false,
      message: '缺少必要參數: u_id, s_id, t_id, vote_type'
    });
  }
  ...
  if (vote_type !== 'good' && vote_type !== 'bad') {
    return res.status(400).json({
      success: false,
      message: 'vote_type 必須是 "good" 或 "bad"'
    });
  }

  // 先檢查是否已有該用戶對此Schedule的評價
  const checkQuery = 'SELECT * FROM Evaluate WHERE u_id = ? AND s_id = ? AND t_id = ?';
  
  connection.query(checkQuery, [u_id, s_id, t_id], (err, results) => {
    if (err) {
      console.error('❌ 檢查評價失敗:', err);
      return res.status(500).json({
        success: false,
        message: '檢查評價失敗',
        error: err.message
    ...
    const isGood = vote_type === 'good';
    const isBad = vote_type === 'bad';

    if (results.length > 0) {
      // 更新現有評價
      const updateQuery = 'UPDATE Evaluate SET good = ?, bad = ? WHERE u_id = ? AND s_id = ? AND t_id = ?';
      connection.query(updateQuery, [isGood, isBad, u_id, s_id, t_id], (updateErr, updateResult) => {
        if (updateErr) {
          console.error('❌ 更新評價失敗:', updateErr);
          return res.status(500).json({
            success: false,
            message: '更新評價失敗',
      ...
      connection.query(insertQuery, [u_id, s_id, t_id, isGood, isBad], (insertErr, insertResult) => {
        if (insertErr) {
          console.error('❌ 新增評價失敗:', insertErr);
          return res.status(500).json({
            success: false,
            message: '新增評價失敗',
...
// API: 取得Schedule的投票統計
app.get('/api/schedule_votes/:s_id', (req, res) => {
  const { s_id } = req.params;
