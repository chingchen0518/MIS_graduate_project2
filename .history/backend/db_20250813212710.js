...
app.get('/api/view3_trip_budget_range/:t_id', (req, res) => {
  ...
  const query = `
    SELECT MIN(a.budget) as min_budget, SUM(a.budget) as max_budget
    FROM attraction a
...
app.get('/api/trip_users/:t_id', (req, res) => {
  ...
  const query = `
    SELECT u.u_id, u.u_name, u.u_img, u.u_email, u.u_account, j.color
    FROM User u
  ...
  connection.query(query, [t_id], (err, results) => {
    ...
    console.log(`✅ 成功取得trip ${t_id} 的使用者數量:`, results.length);
    res.json({
      success: true,
      users: results
    });
