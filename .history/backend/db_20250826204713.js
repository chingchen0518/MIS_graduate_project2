...
// 新增API：刪除投票
app.delete('/api/schedule_vote/:t_id/:s_id/:u_id', (req, res) => {
  ...
  connection.query(deleteQuery, [u_id, s_id, t_id], (err, result) => {
    if (err) {
      ...
      console.log(`Vote deleted for t_id:${t_id}, s_id:${s_id}, u_id:${u_id}`);
...
app.get('/api/user_vote/:t_id/:s_id/:u_id/:date', (req, res) => {
  ...
  connection.query(query, [u_id, s_id, t_id], (err, results) => {
    if (err) {
      ...
      const vote_type = vote.good ? 'like' : (vote.bad ? 'dislike' : null);
