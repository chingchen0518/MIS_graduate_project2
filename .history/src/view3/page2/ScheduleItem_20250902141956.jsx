...
const ScheduleItem = ({ editmode = false, a_id, name, position, targetPosition = null, width, index, s_id, onMove, editable = false, height, onValueChange, onDragStop, intervalHeight, nextAId, isSelected = false, categoryColor, category, isAnimating = false, animationStage = 'idle' }) => {
    ...
    const [isMoving, setIsMoving] = React.useState(false); // 景點移動狀態

    // 當動畫開始且有目標位置時，開始移動動畫
    React.useEffect(() => {
        if (isAnimating && animationStage === 'attractions-moving' && targetPosition) {
            setIsMoving(true);
    ...
    }, [isAnimating, animationStage, targetPosition]);

    // 當 position props 改變時更新位置（非動畫情況）
    React.useEffect(() => {
        if (!isAnimating) {
            setX(position.x);
            setY(position.y);
        }
    }, [position.x, position.y, isAnimating]);
    ...
    return (
        <Rnd
            disableDragging={!editable || isMoving}
            default={{ x: 100, y: 100, width: '100%', height: 35 }}
            position={{ x: x, y: y }}
            size={{ width: 100, height: editable ? heightEdit : height }}
            enableResizing={{ top: editable && !isMoving, bottom: editable && !isMoving, right: false, left: false, topRight: false, bottomRight: false, bottomLeft: false, topLeft: false }}
        ...
        >
            <div
                className={`schedule_item ${isAnimating ? `animating-${animationStage}` : ''}`}
                style={{
                    borderRadius: '8px',
                    padding: '3px', // 邊框厚度
                    ...
                    position: 'relative',
                    transition: isMoving ? 
                        'all 1.5s cubic-bezier(0.4, 0.0, 0.2, 1)' : 
                        'all 0.3s ease',
                    // 漸層背景作為邊框
