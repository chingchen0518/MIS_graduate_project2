...
const ScheduleShow = (props) => {
    ...
    // 從props獲取篩選條件
    const { costRange = [0, 1000], selectedAttractions = [] } = props.filterConditions || {};
    ...
    const getScheduleOpacity = () => {
        if (!isBudgetInRange) return 0.3; // 預算不在範圍內時變透明
        return 1;
    };

    // 檢查特定景點是否被選中
    const isAttractionSelected = (a_id) => {
        console.log('檢查景點選擇狀態:', { 
            a_id, 
            a_id_type: typeof a_id,
            selectedAttractions, 
            selectedAttractions_types: selectedAttractions.map(id => ({ id, type: typeof id })),
            filterConditions: props.filterConditions,
            isSelected: selectedAttractions.includes(a_id),
            isSelectedWithString: selectedAttractions.includes(String(a_id)),
            isSelectedWithNumber: selectedAttractions.includes(Number(a_id))
        });
        // 嘗試兩種比較方式：直接比較和轉換後比較
        return selectedAttractions.includes(a_id) || 
               selectedAttractions.includes(String(a_id)) || 
               selectedAttractions.includes(Number(a_id));
    };
            ...
            <div className="schedule_timeline" style={{ position: 'relative', overflow: 'hidden', maxHeight: props.containerHeight }}>
                ...
                {scheduleItems.map((scheduleItem) => (
                    <ScheduleItem
                        ...
                        isHighlighted={isAttractionSelected(scheduleItem.a_id)}
