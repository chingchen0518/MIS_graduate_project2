...
const ScheduleShow = (props) => {
    ...
    const [animationStage, setAnimationStage] = useState('idle'); // 'idle', 'items_moving', 'schedule_moving'
    ...
    // 檢測排序變化並觸發動畫
    useEffect(() => {
        // 使用 MutationObserver 來檢測DOM位置變化
        const observer = new MutationObserver(() => {
            if (scheduleRef.current && !isAnimating) {
                const currentPrevId = scheduleRef.current.previousElementSibling?.getAttribute('data-schedule-id');
                const currentNextId = scheduleRef.current.nextElementSibling?.getAttribute('data-schedule-id');
                
                if (previousOrder && 
                    (currentPrevId !== previousOrder.prevId || currentNextId !== previousOrder.nextId)) {
                    startSortingAnimation();
                }
                
                // 更新當前位置記錄
                setPreviousOrder({
                    prevId: currentPrevId || null,
                    nextId: currentNextId || null
                });
            }
        });

        // 觀察父容器的子元素變化
        const parentElement = scheduleRef.current?.parentElement;
        if (parentElement) {
    ...
    }, [isAnimating, previousOrder, startSortingAnimation]);

    // 記錄初始位置順序
    useEffect(() => {
        if (scheduleRef.current && !previousOrder) {
            const prevElement = scheduleRef.current.previousElementSibling;
            const nextElement = scheduleRef.current.nextElementSibling;
            
            setPreviousOrder({
                prevId: prevElement?.getAttribute('data-schedule-id') || null,
                nextId: nextElement?.getAttribute('data-schedule-id') || null
            });
        }
    }, [props.s_id, previousOrder]);

    // 啟動排序動畫
    const startSortingAnimation = useCallback(() => {
        if (isAnimating) return; // 防止重複動畫
        ...
        setIsAnimating(true);
        setAnimationStage('items_moving');
        
        // 第一階段：景點直線移動動畫 (1秒)
        animationTimeoutRef.current = setTimeout(() => {
            setAnimationStage('schedule_moving');
    ...
    const getAnimationStyle = useCallback(() => {
        ...
        if (animationStage === 'items_moving') {
            return {
                ...baseStyle,
                transform: 'translateX(0)',
                transition: 'transform 1s cubic-bezier(0.4, 0, 0.2, 1)'
            };
        } else if (animationStage === 'schedule_moving') {
            return {
                ...baseStyle,
                transform: 'translateX(0)',
    ...
    }, [animationStage, getScheduleOpacity]);
    ...
    // 獲取動態CSS類名
    const getScheduleClassName = useCallback(() => {
        let className = 'schedule scheduleShow';
        
        if (isAnimating) {
            className += ' animating';
        }
        
        if (animationStage === 'schedule_moving') {
            className += ' moving';
        }
    ...
    return (
        ...
        <div
            ref={scheduleRef}
            className={getScheduleClassName()}
                ...
                {scheduleItems.map((scheduleItem) => (
                    ...
                    >
                        <ScheduleItem
                            ...
                            animationStyle={getItemAnimationStyle()} // 傳遞動畫樣式
                        />
                    </div>
                ))}
            </div>
