...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    }, [isReady, schedules, svgDimensions, hoveredGroup]);
    ...
    const drawConnections = () => {
        ...
        // 如果有懸停的組，繪製貫穿線
        if (hoveredGroup) {
            drawThroughLine(svg, hoveredGroup);
        }
    ...
    const drawConnectionPath = (svg, pathData, hasIntersection) => {
        ...
        targetGroup.append('path')
            ...
            .attr('data-attraction-name', pathData.attractionName) // 添加景點名稱標識
    ...
    // 新增：繪製貫穿線的函數
    const drawThroughLine = (svg, attractionName) => {
        const attractionPaths = attractionGroupsRef.current[attractionName];
        if (!attractionPaths || attractionPaths.length === 0) return;

        // 使用主要群組繪製
        const targetGroup = svg.mainGroup || svg;

        // 計算貫穿線的起點和終點 - 找到整個景點組的最左到最右的範圍
        let leftmostX = Infinity, rightmostX = -Infinity;
        let leftmostY = 0, rightmostY = 0;
        let totalPaths = 0;

        attractionPaths.forEach(pathData => {
            const leftX = Math.min(pathData.fromX, pathData.toX);
            const rightX = Math.max(pathData.fromX, pathData.toX);
            const leftY = leftX === pathData.fromX ? 
                (pathData.fromYTop + pathData.fromYBottom) / 2 : 
                (pathData.toYTop + pathData.toYBottom) / 2;
            const rightY = rightX === pathData.toX ? 
                (pathData.toYTop + pathData.toYBottom) / 2 : 
                (pathData.fromYTop + pathData.fromYBottom) / 2;
        ...
        // 繪製粗的貫穿線
        if (totalPaths > 0 && leftmostX !== Infinity && rightmostX !== -Infinity) {
            targetGroup.append('line')
                .attr('class', 'through-line')
                .attr('x1', leftmostX)
                .attr('y1', leftmostY)
                .attr('x2', rightmostX)
                .attr('y2', rightmostY)
                .style('stroke', '#FFD700') // 金色
                .style('stroke-width', '4px')
                .style('stroke-opacity', '0.8')
                .style('stroke-linecap', 'round')
                .style('pointer-events', 'none') // 不干擾其他互動
