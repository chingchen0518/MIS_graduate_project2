...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    const drawConnectionPath = (svg, pathData, hasIntersection, attractionGroups) => {
        ...
        const pathElement = targetGroup.append('path')
            ...
            .style('fill-opacity', hoveredGroup === null ? '0.3' : 
                (hoveredGroup === pathData.attractionName ? '0.6' : '0.1')) // 動態透明度
            .style('stroke', color) // 邊框同色
            .style('stroke-width', '1px') // 細邊框
            .style('stroke-opacity', hoveredGroup === null ? '0.8' : 
                (hoveredGroup === pathData.attractionName ? '1' : '0.3')) // 動態邊框透明度
            .style('pointer-events', 'all') // 允許滑鼠事件
            .style('cursor', 'pointer')
            .style('transition', 'all 0.2s ease') // 平滑過渡
            .on('mouseenter', () => handleGroupHover(pathData.attractionName))
            .on('mouseleave', () => handleGroupHover(null));
    };

    const handleGroupHover = (attractionName) => {
        setHoveredGroup(attractionName);
        ...
        svg.selectAll('.attraction-connection')
            .style('fill-opacity', attractionName === null ? '0.3' : 
                function() {
                    const elementAttractionName = d3.select(this).attr('data-attraction-name');
                    return elementAttractionName === attractionName ? '0.6' : '0.1';
                })
            .style('stroke-opacity', attractionName === null ? '0.8' : 
                function() {
                    const elementAttractionName = d3.select(this).attr('data-attraction-name');
                    return elementAttractionName === attractionName ? '1' : '0.3';
                });
    };
