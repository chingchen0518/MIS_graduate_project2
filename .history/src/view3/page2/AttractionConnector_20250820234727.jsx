...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    const isConnectionVisible = (from, to) => {
        ...
        const margin = 50; // 添加邊界緩衝區
        
        // 檢查起點是否完全在可視區域內（包括緩衝區）
        const fromVisible = from.left >= -margin && from.right <= viewportWidth + margin && 
                           from.top >= -margin && from.bottom <= viewportHeight + margin;
        
        // 檢查終點是否完全在可視區域內（包括緩衝區）
        const toVisible = to.left >= -margin && to.right <= viewportWidth + margin && 
                         to.top >= -margin && to.bottom <= viewportHeight + margin;
        
        // 只有當兩個端點都在可視區域內時才繪製連線
        const bothVisible = fromVisible && toVisible;
        
        console.log(`👁️ 可見性檢查: ${from.name} -> ${to.name}`, {
            fromVisible,
            toVisible,
            bothVisible,
    ...
    const drawConnectionPath = (svg, pathData, hasIntersection) => {
        ...
        svg.append('path')
            ...
            .attr('clip-path', 'url(#viewport-clip)') // 應用裁剪路徑
