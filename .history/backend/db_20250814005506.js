...
app.get('/api/schedule_budget/:t_id/:s_id/:date', (req, res) => {
  ...
  console.log(`計算預算 - t_id: ${t_id}, s_id: ${s_id}, date: ${date}`);

  // 首先檢查該日期的Schedule是否存在
  const checkScheduleQuery = `SELECT s_id FROM Schedule WHERE t_id = ? AND s_id = ? AND date = ?`;
  
  connection.query(checkScheduleQuery, [t_id, s_id, date], (err, scheduleResults) => {
    if (err) {
      console.error('Error checking schedule:', err);
      return res.status(200).json({ total_budget: 0 });
    }
    
    if (scheduleResults.length === 0) {
      console.log(`No schedule found for t_id:${t_id}, s_id:${s_id}, date:${date}`);
      return res.status(200).json({ total_budget: 0 });
    }
    
    // 計算該Schedule的所有景點預算總和
    const budgetQuery = `
      SELECT SUM(a.budget) as total_budget 
      FROM Schedule s
    ...
    connection.query(budgetQuery, values, (err, results) => {
      if (err) {
        console.error('Error calculating budget:', err);
        res.status(200).json({ total_budget: 0 });
      } else {
        const totalBudget = results[0]?.total_budget || 0;
        console.log(`Budget calculation result for t_id:${t_id}, s_id:${s_id}, date:${date} = ${totalBudget}`);
