...
const ScheduleShow = (props) => {
    ...
    useEffect(() => {
        ...
        fetch(api)
            ...
            .then((data) => {
                // 檢查是否有序列變化需要動畫
                if (scheduleItems.length > 0 && data.length > 0) {
                    const hasSequenceChange = data.some((newItem) => {
                        const oldItem = scheduleItems.find(item => item.a_id === newItem.a_id);
                        return oldItem && (oldItem.sequence !== newItem.sequence || oldItem.y !== newItem.y);
                    });

                    if (hasSequenceChange) {
                        setPrevScheduleItems([...scheduleItems]); // 保存舊狀態
