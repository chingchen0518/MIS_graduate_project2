...
const ScheduleShow = (props) => {
    ...
    // 計算Schedule的透明度（基於預算篩選）
    const getScheduleOpacity = () => {
        if (!props.filterConditions?.budgetRange) return 1;
        
        const { min, max } = props.filterConditions.budgetRange;
        const scheduleBudget = 350; // 這裡應該從API獲取真實預算
        
        if (scheduleBudget >= min && scheduleBudget <= max) {
            return 1; // 在範圍內，完全不透明
        } else {
            return 0.3; // 在範圍外，變透明
        }
        ...
        >
            ...
            <div className="schedule_timeline" style={{ position: 'relative', overflow: 'hidden', maxHeight: props.containerHeight }}>
                ...
                {scheduleItems.map((scheduleItem) => {
                    // 檢查該景點是否被選中
                    const isSelected = props.filterConditions?.selectedAttractions?.includes(scheduleItem.a_id) || false;
                    ...
                    return (
                        <ScheduleItem
                            ...
                            isSelected={isSelected} // 傳遞是否被選中的狀態
                        />
                    );
                })}
            </div>
