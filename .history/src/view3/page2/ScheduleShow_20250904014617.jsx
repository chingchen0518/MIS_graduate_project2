...
const ScheduleShow = (props) => {
    ...
    useEffect(() => {
        ...
        if (props.t_id && props.s_id && props.date && currentUserId) {
            fetchVotes();
        }
    }, [props.t_id, props.s_id, props.date, formatDate, currentUserId]);

    // 投票處理函數
    const handleVote = useCallback(async (voteType) => {
        if (!currentUserId) {
            // 如果沒有用戶信息，可以顯示登入提示或直接返回
            return;
        }
    ...
    }, [currentUserVote, props.t_id, props.s_id, props.date, formatDate, currentUserId]); // 添加 currentUserId 到依賴項
