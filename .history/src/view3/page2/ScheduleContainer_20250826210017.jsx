    ...
    const calculateScheduleStats = async (schedule) => {
        try {
            ...
            if (response.ok) {
                ...
                // 計算總遊玩時間 (y軸總長度)
                const totalDuration = scheduleItems.reduce((sum, item) => sum + (item.height || 60), 0);
                
                // 計算類別數量 (使用景點的category屬性)
                const categories = new Set(scheduleItems.map(item => item.category || '其他').filter(Boolean));
                const categoriesCount = categories.size;
                
                console.log(`📊 統計資料 - 行程 ${schedule.s_id}:`, {
                    attractionsCount,
                    totalDuration,
                    categoriesCount,
