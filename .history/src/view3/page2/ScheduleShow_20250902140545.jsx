...
const ScheduleShow = (props) => {
    ...
    const [prevScheduleItems, setPrevScheduleItems] = useState([]); // 新增：保存前一次的狀態
    const [isAnimating, setIsAnimating] = useState(false); // 新增：動畫狀態
    ...
    // 使用 ref 來保存前一次的 scheduleItems，避免依賴循環
    const prevScheduleItemsRef = useRef([]);

    // 從 props 接收回調函數
    const { onAttractionSelect, onShowRoute, onHideRoute } = props;
    ...
    useEffect(() => {
        ...
        fetch(api)
            ...
            .then((data) => {
                // 檢查是否有序列變化需要動畫
                const prevItems = prevScheduleItemsRef.current;
                if (prevItems.length > 0 && data.length > 0) {
                    const hasSequenceChange = data.some((newItem) => {
                        const oldItem = prevItems.find(item => item.a_id === newItem.a_id);
                        return oldItem && (oldItem.sequence !== newItem.sequence || Math.abs(oldItem.y - newItem.y) > 5);
                    });

                    if (hasSequenceChange) {
                        setPrevScheduleItems([...prevItems]); // 保存舊狀態到 state 用於渲染
                        setIsAnimating(true); // 開始動畫
                        
                        // 延遲設置新數據，讓動畫有時間開始
                        setTimeout(() => {
                            setScheduleItems(data);
                            prevScheduleItemsRef.current = data; // 更新 ref
                            
                            // 動畫結束後重置狀態
                            setTimeout(() => {
                                setIsAnimating(false);
                                setPrevScheduleItems([]);
                            }, 800); // 與 CSS 動畫時間匹配
                        }, 50);
                    } else {
                        setScheduleItems(data);
    ...
    }, [props.t_id, props.s_id]); // 移除 scheduleItems.length 避免循環依賴

    // Use Effect 2.5: 處理動畫完成後的狀態
    useEffect(() => {
        if (isAnimating) {
            // 動畫開始後一段時間，讓新項目淡入
            const timer1 = setTimeout(() => {
                // 觸發新項目的淡入動畫
                const newItems = document.querySelectorAll('.schedule-item-current.animating');
    ...
    return (
        //層級1：單個schedule 
        <div
            className={`schedule scheduleShow ${isAnimating ? 'animating' : ''}`}
        ...
        >
            ...
            {/* //層級3：schedule放内容的地方 */}
            <div className={`schedule_timeline ${isAnimating ? 'animating' : ''}`} style={{ position: 'relative', overflow: 'hidden', maxHeight: props.containerHeight }}>
                {renderGrid()}

                {/* 顯示景點（已經在資料庫的） */}
                {/* 如果正在動畫中，同時顯示舊的和新的項目 */}
                {isAnimating && prevScheduleItems.map((scheduleItem) => (
                    <div
                        key={`old-schedule-item-wrapper-${scheduleItem.id}-${scheduleItem.a_id}`}
                        className="schedule-item-old"
                        style={{
                            position: 'absolute',
                            left: scheduleItem.x || 0,
                            top: scheduleItem.y || 0,
                            ...
                            filter: 'blur(0px)',
                            zIndex: 1,
                            pointerEvents: 'none' // 防止點擊干擾
                        }}
                    >
                        <ScheduleItem
                            key={`old-schedule-item-${scheduleItem.id}-${scheduleItem.a_id}`}
                            s_id={scheduleItem.id}
                            name={scheduleItem.name}
                            position={{ x: 0, y: 0 }} // 位置由外層容器控制
                            width={scheduleWidths}
                            height={scheduleItem.height}
                            editable={false}
                            ...
                            isSelected={selectedAttractions.includes(scheduleItem.a_id)}
                            categoryColor={getCategoryColor(scheduleItem.category)}
                            category={scheduleItem.category}
                            isAnimating={false} // 舊項目不需要特別的動畫
                        />
                    </div>
                ))}
                ...
                {scheduleItems.map((scheduleItem) => (
                    <div
                        ...
                        className={`schedule-item-current ${isAnimating ? 'animating' : ''}`}
                        style={{ 
                            cursor: 'pointer',
                            position: 'absolute',
                            left: scheduleItem.x || 0,
                    ...
                    >
                        <ScheduleItem
                            ...
                            isAnimating={isAnimating} // 傳遞動畫狀態
                        />
                    </div>
                ))}
            </div>
