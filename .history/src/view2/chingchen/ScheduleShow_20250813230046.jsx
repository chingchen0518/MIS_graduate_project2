...
const ScheduleShow = (props) => {
    ...
    const [voteStatus, setVoteStatus] = useState({ good: false, bad: false });
    ...
    const handleVote = async (voteType) => {
        try {
            const response = await fetch(`http://localhost:3001/api/schedule_vote`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    u_id: 1, // 假設當前使用者ID為1，實際應該從登入狀態獲取
                    s_id: props.s_id,
                    t_id: props.t_id,
                    good: voteType === 'good',
                    bad: voteType === 'bad'
            ...
            if (response.ok) {
                setVoteStatus({
                    good: voteType === 'good' ? !voteStatus.good : false,
                    bad: voteType === 'bad' ? !voteStatus.bad : false
                });
            }
        } catch (error) {
