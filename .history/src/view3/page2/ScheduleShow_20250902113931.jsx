...
const ScheduleShow = (props) => {
        ...
        const fetchVotes = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    const errorText = await response.text();
                    console.log('獲取投票數據失敗:', errorText);
                }
                ...
                if (userVoteResponse.ok) {
                    ...
                    console.log('用戶投票狀態:', userVoteData);
                    setCurrentUserVote(userVoteData.vote_type || null);
                } else {
                    console.log('獲取用戶投票狀態失敗');
                    setCurrentUserVote(null);
                }
            } catch (error) {
                console.error('獲取投票信息時發生錯誤:', error);
            }
        };
    ...
    const handleVote = useCallback(async (voteType) => {
        try {
            ...
            console.log('投票前狀態:', { currentUserVote, voteType });

            // 如果點擊的是已經投過的票，則取消投票
            const finalVoteType = currentUserVote === voteType ? null : voteType;

            console.log('最終投票類型:', finalVoteType);
            ...
            if (response.ok) {
                ...
                if (userVoteResponse.ok) {
                    const userVoteData = await userVoteResponse.json();
                    console.log('更新後的用戶投票狀態:', userVoteData);
                    setCurrentUserVote(userVoteData.vote_type || null);
                }
                ...
                console.error('投票失敗:', await response.text());
            }
        } catch (error) {
