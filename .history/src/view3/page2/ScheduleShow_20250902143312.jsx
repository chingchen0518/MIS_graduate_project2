...
const ScheduleShow = (props) => {
    ...
    // 檢測排序變化並觸發動畫
    useEffect(() => {
        if (props.isReordering && !isAnimating) {
            // 記錄當前位置
    ...
    }, [props.isReordering, props.sortIndex, isAnimating, startSortingAnimation]);

    // 記錄初始位置
    useEffect(() => {
        if (scheduleRef.current && !initialPosition) {
            const rect = scheduleRef.current.getBoundingClientRect();
            setInitialPosition({
                left: rect.left,
                top: rect.top
    ...
    const getItemAnimationStyle = useCallback(() => {
        if (animationStage === 'items_moving') {
            return {
                ...
                transform: 'translateY(-10px) translateX(5px)', // 先向上再向右的直線移動
                zIndex: 10 // 提高層級，讓移動更明顯
            };
        } else if (animationStage === 'schedule_moving') {
            return {
                transition: 'all 0.5s cubic-bezier(0.4, 0, 0.2, 1)',
                transform: 'translateY(0) translateX(0)', // 回到正確位置
                zIndex: 1
            };
        }
        return {
    ...
    // 獲取景點容器的CSS類名
    const getItemWrapperClassName = useCallback(() => {
        let className = 'schedule-item-wrapper';
        
        if (animationStage === 'items_moving') {
            className += ' items-moving';
        } else if (animationStage === 'schedule_moving') {
            className += ' schedule-moving';
        }
            ...
            <div className="schedule_timeline" style={{ position: 'relative', overflow: 'hidden', maxHeight: props.containerHeight }}>
                ...
                {scheduleItems.map((scheduleItem) => (
                    <div
                        ...
                        className={getItemWrapperClassName()}
