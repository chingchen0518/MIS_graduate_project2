...
app.get('/api/schedule_budget/:t_id/:s_id/:date', (req, res) => {
  ...
  // 首先根據日期和t_id查詢Schedule，確認該s_id在指定日期存在
  const scheduleQuery = `SELECT s_id FROM Schedule WHERE t_id = ? AND date = ? AND s_id = ?`;
  const scheduleValues = [t_id, date, s_id];

  connection.query(scheduleQuery, scheduleValues, (err, scheduleResults) => {
    if (err) {
    ...
    // 如果找到對應的Schedule，則查詢該Schedule的景點預算總額
    const budgetQuery = `SELECT SUM(a.budget) as total_budget 
                         FROM Schedule_include si
                         JOIN Attraction a ON si.a_id = a.a_id
                         WHERE si.t_id = ? AND si.s_id = ?`;
    const budgetValues = [t_id, s_id];

    connection.query(budgetQuery, budgetValues, (err, budgetResults) => {
      if (err) {
        ...
        console.log(`Budget calculation for t_id:${t_id}, s_id:${s_id}, date:${date} = ${totalBudget}`);
