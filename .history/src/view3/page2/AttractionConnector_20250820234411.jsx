...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    const isConnectionVisible = (from, to) => {
        ...
        // 檢查起點是否可見
        const fromVisible = from.left < viewportWidth && from.right > 0 && 
                           from.top < viewportHeight && from.bottom > 0;
        
        // 檢查終點是否可見
        const toVisible = to.left < viewportWidth && to.right > 0 && 
                         to.top < viewportHeight && to.bottom > 0;
