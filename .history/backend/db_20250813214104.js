...
app.get('/api/attraction_categories/:t_id', (req, res) => {
  ...
  const query = `
    ...
    FROM Atraction a
    INNER JOIN Shedule_include si ON a.a_id = si.a_id
...
// API: 取得指定trip的景點預算範圍（改為計算每個Schedule的景點預算加總的最大值）
app.get('/api/view3_trip_budget_range/:t_id', (req, res) => {
  const { t_id } = req.params;

  const query = `
    SELECT 
      MIN(a.budget) as min_budget,
      MAX(schedule_total.total_budget) as max_budget
    FROM attraction a
    ...
    INNER JOIN (
      ...
      GROUP BY s.s_id
    ) as schedule_total ON s.s_id = schedule_total.s_id
    WHERE s.t_id = ? AND a.budget IS NOT NULL
  `;

  connection.query(query, [t_id, t_id], (err, results) => {
    if (err) {
...
// API: 取得指定trip的參與使用者（通過Join表）
app.get('/api/trip_participants/:t_id', (req, res) => {
  const { t_id } = req.params;
  ...
  connection.query(query, [t_id], (err, results) => {
    if (err) {
      console.error('❌ 取得trip參與者時出錯：', err.message);
      res.status(500).json({ error: err.message });
      return;
    }

    console.log(`✅ 成功取得trip ${t_id} 的參與者：`, results);
    res.json({
      success: true,
