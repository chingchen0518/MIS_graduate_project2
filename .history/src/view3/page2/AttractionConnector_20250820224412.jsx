...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    const calculateConnectionPaths = (from, to) => {
        ...
        // 確保足夠的間距，讓上下兩條線完全分開
        const verticalGap = Math.max(from.height * 0.6, 20); // 至少20px的間距
        
        // 上方連線：從起點上方直接連到終點上方
        const fromYTop = from.top + from.height * 0.15; // 接近頂部
        const toYTop = to.top + to.height * 0.15; // 接近頂部
        
        // 下方連線：從起點下方直接連到終點下方
        const fromYBottom = from.bottom - from.height * 0.15; // 接近底部
        const toYBottom = to.bottom - to.height * 0.15; // 接近底部
        
        // 計算弧形曲線的控制點 - 保持上下線條完全分離
        const horizontalDistance = toX - fromX;
        const curvature = Math.max(horizontalDistance * 0.4, 40); // 增加曲率讓線條更平滑
        ...
        return [
            {
                ...
                curvature,
                // 上方線條的控制點保持在上方
                path: createSeparateCurvePath(fromX, fromYTop, toX, toYTop, curvature, 'top')
            },
            {
                type: 'bottom', 
                ...
                curvature,
                // 下方線條的控制點保持在下方
                path: createSeparateCurvePath(fromX, fromYBottom, toX, toYBottom, curvature, 'bottom')
            }
        ];
    };

    const createSeparateCurvePath = (fromX, fromY, toX, toY, curvature, type) => {
        const path = d3.path();
        path.moveTo(fromX, fromY);
        
        // 根據線條類型調整控制點，確保上下線條不會交匯
        const controlOffset = type === 'top' ? -10 : 10; // 上方線控制點向上偏移，下方線向下偏移
        
        path.bezierCurveTo(
            fromX + curvature, fromY + controlOffset,    // 第一個控制點帶偏移
            toX - curvature, toY + controlOffset,        // 第二個控制點帶偏移
            toX, toY                                     // 終點
        );
