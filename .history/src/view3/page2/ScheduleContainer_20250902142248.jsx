...
const ScheduleContainer = ({ t_id, usedAttractions = [], onAttractionUsed, onShowRoute, onHideRoute, onAttractionSelect, filterConditions }) => {
    ...
    const [isReordering, setIsReordering] = useState(false); // 重新排序狀態
    ...
    const handleSortChange = (event) => {
        ...
        // 只有在排序方式真正改變時才觸發動畫
        if (newSortBy !== sortBy && schedules.length > 0) {
    ...
    }, [selectedDate]); // 移除 sortBy 依賴

    //【useEffect 3】當排序方式改變時重新排序現有數據（不重新獲取）
    useEffect(() => {
        const applySorting = async () => {
            if (schedules.length > 0) {
                // 不要設置 loading，直接排序現有數據
