...
// API：獲取行程統計數據 (景點數量、總遊玩時間、類別數量)
app.get('/api/schedule_stats/:s_id/:date', (req, res) => {
  const { s_id, date } = req.params;
  
  // 查詢該行程在特定日期的景點統計數據
  const query = `
    SELECT 
      COUNT(DISTINCT si.a_id) as attraction_count,
      SUM(COALESCE(si.height, 0)) as total_duration,
      COUNT(DISTINCT a.type) as category_count
    FROM Schedule_include si
    LEFT JOIN Attraction a ON si.a_id = a.a_id
    LEFT JOIN Schedule s ON si.s_id = s.s_id
    WHERE si.s_id = ? 
    AND DATE(s.date) = DATE(?)
  `;
  
  connection.query(query, [s_id, date], (err, results) => {
    if (err) {
      console.error('❌ 查詢行程統計失敗：', err.message);
      return res.status(500).json({ error: '伺服器錯誤' });
    }
    ...
    // 將 total_duration 轉換為小時 (假設 height 是以分鐘為單位)
    stats.total_duration = Math.round((stats.total_duration || 0) / 60 * 100) / 100;
    
    console.log(`📊 行程 ${s_id} 統計:`, stats);
    res.json(stats);
  });
});
