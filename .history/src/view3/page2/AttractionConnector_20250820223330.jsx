...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    const drawConnection = (svg, from, to) => {
        ...
        // 計算上下連接點的Y座標
        const offsetY = from.height * 0.25; // 景點高度的1/4作為上下偏移
        
        // 上方連線的起點和終點
        const fromYTop = from.top + offsetY;
        const toYTop = to.top + offsetY;
        
        // 下方連線的起點和終點
        const fromYBottom = from.bottom - offsetY;
        const toYBottom = to.bottom - offsetY;
