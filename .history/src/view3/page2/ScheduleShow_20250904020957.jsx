...
const ScheduleShow = (props) => {
    ...
    const handleVote = useCallback(async (voteType) => {
        try {
            ...
            const response = await fetch(`http://localhost:3001/api/schedule_vote/${tripId}/${props.s_id}/${currentUserId}/${formattedDate}`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                // 重新獲取投票數據
                const voteResponse = await fetch(`http://localhost:3001/api/schedule_votes/${tripId}/${props.s_id}/${formattedDate}`);
                if (voteResponse.ok) {
    ...
    }, [currentUserVote, props.t_id, props.s_id, props.date, formatDate, user?.uid, trip?.tid]); // 添加 user?.uid 到依賴項
