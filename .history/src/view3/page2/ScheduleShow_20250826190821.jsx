                    ...
                    <div className="vote_buttons">
                        <div className="vote_button_container">
                            <button
                                ...
                                title={`讚 (${voteData.total_likes}票)`}
                            >
                                👍
                            </button>
                            {voteData.total_likes > 0 && (
                                <div 
                                    className="vote_glow like_glow" 
                                    style={{ 
                                        '--vote-count': Math.min(voteData.total_likes, 15),
                                        '--glow-size': `${32 + Math.min(voteData.total_likes * 4, 20)}px`,
                                        '--glow-opacity': Math.min(0.4 + voteData.total_likes * 0.15, 1),
                                        '--border-width': `${Math.min(2 + Math.floor(voteData.total_likes / 3), 4)}px`
                                    }}
                                ></div>
                            )}
                        </div>
                        <div className="vote_button_container">
                            <button
                                ...
                                title={`倒讚 (${voteData.total_dislikes}票)`}
                            >
                                👎
                            </button>
                            {voteData.total_dislikes > 0 && (
                                <div 
                                    className="vote_glow dislike_glow" 
                                    style={{ 
                                        '--vote-count': Math.min(voteData.total_dislikes, 15),
                                        '--glow-size': `${32 + Math.min(voteData.total_dislikes * 4, 20)}px`,
                                        '--glow-opacity': Math.min(0.4 + voteData.total_dislikes * 0.15, 1),
                                        '--border-width': `${Math.min(2 + Math.floor(voteData.total_dislikes / 3), 4)}px`
                                    }}
                                ></div>
                            )}
                        </div>
