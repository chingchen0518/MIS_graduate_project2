...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    // 當懸停狀態變化時，只更新樣式，不重新繪製
    useEffect(() => {
        if (!svgRef.current) return;
        
        updateConnectionStyles(hoveredAttraction);
    }, [hoveredAttraction]);

    // 更新連線樣式的函數（不重新繪製，只改變樣式）
    const updateConnectionStyles = (hoveredAttraction) => {
        if (!svgRef.current) return;

        const svg = d3.select(svgRef.current);
        const connections = svg.selectAll('.attraction-connection');

        connections.each(function() {
            const path = d3.select(this);
            const attractionName = path.attr('data-attraction-name');
            ...
            if (hoveredAttraction !== null) {
                if (attractionName === hoveredAttraction) {
                    // 被懸停的景點線條發亮
                    opacity = 0.7;
                    strokeOpacity = 1.0;
                } else {
    ...
    const drawConnection = (svg, from, to) => {
        ...
        paths.forEach(pathData => {
            drawConnectionPath(svg, pathData, false, hoveredAttraction);
        });
    };
