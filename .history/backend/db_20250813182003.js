...
app.get('/api/attraction-categories', (req, res) => {
  ...
  db.query(query, (err, results) => {
    if (err) {
      console.error('獲取景點類別失敗:', err);
      res.status(500).json({ error: '獲取景點類別失敗' });
      return;
    }
...
// API：獲取所有城市
app.get('/api/attraction-cities', (req, res) => {
  const query = `
  ...
  db.query(query, (err, results) => {
    if (err) {
      console.error('獲取城市失敗:', err);
      res.status(500).json({ error: '獲取城市失敗' });
      return;
    }
