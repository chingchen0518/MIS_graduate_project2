...
const ScheduleShow = (props) => {
    ...
    const [pendingScheduleItems, setPendingScheduleItems] = useState([]); // 新的景點位置數據
    ...
    useEffect(() => {
        if (isReordering) {
            ...
            // 根據行程索引調整延遲時間，創造交錯動畫效果
            const delay = scheduleIndex * 200;
    ...
    useEffect(() => {
        ...
        fetch(api)
            ...
            .then((data) => {
                // 檢測是否需要動畫（景點位置有變化且不是第一次載入）
                const hasExistingItems = scheduleItems.length > 0;
                const hasNewData = data.length > 0;
                const positionsChanged = hasExistingItems && hasNewData && 
                                       ...
                                       JSON.stringify(scheduleItems.map(item => ({ 
                                           a_id: item.a_id, 
                                           x: item.x, 
                                           y: item.y, 
                                           sequence: item.sequence 
                ...
                if (positionsChanged) {
                    ...
                    // 1500ms 後景點移動到新位置
                    setTimeout(() => {
                        setScheduleItems(data); // 更新景點到新位置
                        setAnimationStage('schedule-sliding');
    ...
    return (
        ...
        <div
            ...
            style={{
                ...
                transition: isAnimating ? 'opacity 0.3s ease' : 'opacity 0.3s ease',
                cursor: 'pointer'
            }}
        ...
        >
            ...
            <div className="schedule_timeline" style={{ position: 'relative', overflow: 'hidden', maxHeight: props.containerHeight }}>
                ...
                {scheduleItems.map((scheduleItem) => {
                    // 在動畫期間，使用 pendingScheduleItems 來獲取目標位置
                    const targetItem = isAnimating && pendingScheduleItems.length > 0 
                        ? pendingScheduleItems.find(pending => pending.a_id === scheduleItem.a_id) 
                        : null;
                    
                    const targetPosition = targetItem ? { x: targetItem.x, y: targetItem.y } : null;
                    ...
                    return (
                        ...
                        >
                            <ScheduleItem
                                ...
                                position={{ x: scheduleItem.x, y: scheduleItem.y }} // 當前位置
                                targetPosition={targetPosition} // 目標位置
