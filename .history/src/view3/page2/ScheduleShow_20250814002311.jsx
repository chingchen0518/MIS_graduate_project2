...
const ScheduleShow = (props) => {
    ...
    // 添加調試信息
    useEffect(() => {
        console.log('ScheduleShow props:', { t_id: props.t_id, s_id: props.s_id, title: props.title });
    ...
    useEffect(() => {
        const fetchBudget = async () => {
            try {
                console.log(`Fetching budget for t_id: ${props.t_id}, s_id: ${props.s_id}, date: ${props.date}`);
                const response = await fetch(`http://localhost:3001/api/schedule_budget/${props.t_id}/${props.s_id}/${props.date}`);
    ...
    useEffect(() => {
        const fetchVotes = async () => {
            try {
                console.log(`Fetching votes for t_id: ${props.t_id}, s_id: ${props.s_id}, date: ${props.date}`);
                const response = await fetch(`http://localhost:3001/api/schedule_votes/${props.t_id}/${props.s_id}/${props.date}`);
    ...
    const handleVote = async (voteType) => {
        try {
            ...
            const response = await fetch(`http://localhost:3001/api/schedule_vote/${props.t_id}/${props.s_id}/${currentUserId}/${props.date}`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                // 重新獲取投票數據
                const voteResponse = await fetch(`http://localhost:3001/api/schedule_votes/${props.t_id}/${props.s_id}/${props.date}`);
                if (voteResponse.ok) {
