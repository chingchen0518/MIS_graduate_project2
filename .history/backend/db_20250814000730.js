...
// 測試API：檢查Schedule_include表的數據
app.get('/api/test_schedule_include/:t_id/:s_id', (req, res) => {
  const { t_id, s_id } = req.params;
  
  console.log(`[測試API] 檢查 Schedule_include - t_id: ${t_id}, s_id: ${s_id}`);

  const query1 = `SELECT * FROM Schedule_include WHERE t_id = ? AND s_id = ?`;
  
  connection.query(query1, [t_id, s_id], (err, scheduleResults) => {
    if (err) {
      console.error('[測試API] Schedule_include查詢錯誤:', err);
      return res.status(500).json({ error: err.message });
    }
    
    console.log(`[測試API] Schedule_include結果:`, scheduleResults);
    
    if (scheduleResults.length === 0) {
      return res.json({ 
        message: 'No Schedule_include records found',
        schedule_include: [],
        attractions: []
      });
    }
    ...
    // 獲取對應的景點資料
    const attractionIds = scheduleResults.map(item => item.a_id);
    const query2 = `SELECT a_id, name, budget FROM Attraction WHERE a_id IN (${attractionIds.map(() => '?').join(',')})`;
    
    connection.query(query2, attractionIds, (err2, attractionResults) => {
      if (err2) {
        console.error('[測試API] Attraction查詢錯誤:', err2);
        return res.status(500).json({ error: err2.message });
      }
      
      console.log(`[測試API] Attraction結果:`, attractionResults);
      
      res.json({
        schedule_include: scheduleResults,
        attractions: attractionResults,
        total_budget: attractionResults.reduce((sum, item) => sum + (item.budget || 0), 0)
      });
    });
  });
});
...
// 新增API：計算特定行程的總預算
app.get('/api/schedule_budget/:t_id/:s_id', (req, res) => {
  const { t_id, s_id } = req.params;
  
  console.log(`[預算API] 收到請求 - t_id: ${t_id}, s_id: ${s_id}`);

  const query = `SELECT SUM(a.budget) as total_budget 
                 FROM Schedule_include si
                 JOIN Attraction a ON si.a_id = a.a_id
                 WHERE si.t_id = ? AND si.s_id = ?`;
  const values = [t_id, s_id];

  console.log(`[預算API] 執行查詢:`, query);
  console.log(`[預算API] 參數:`, values);

  connection.query(query, values, (err, results) => {
    if (err) {
      console.error('[預算API] 查詢錯誤:', err);
      res.status(200).json({ total_budget: 0 });
    } else {
      console.log(`[預算API] 查詢結果:`, results);
      const totalBudget = results[0]?.total_budget || 0;
      console.log(`[預算API] 最終預算:`, totalBudget);
...
// 新增API：獲取行程投票狀態
app.get('/api/schedule_votes/:t_id/:s_id', (req, res) => {
  const { t_id, s_id } = req.params;
  ...
  connection.query(query, [t_id, s_id], (err, results) => {
    if (err) {
      ...
      res.status(500).send('Failed to fetch vote data');
    } else {
      const votes = {
...
// 新增API：投票給行程
app.post('/api/schedule_vote/:t_id/:s_id/:u_id', (req, res) => {
