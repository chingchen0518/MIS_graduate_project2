...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    const drawConnections = () => {
        ...
        svg
            ...
            .style('pointer-events', 'auto') // 改為 auto 以允許子元素的事件
            .style('z-index', '1000')
    ...
    const drawConnectionPath = (svg, pathData, hasIntersection) => {
        ...
        const pathElement = targetGroup.append('path')
            ...
            .attr('data-attraction-group', pathData.attractionName) // 添加景點組標識
            ...
            .style('pointer-events', 'all') // 啟用滑鼠事件
        ...
        pathElement
            .on('mouseenter', function(event, d) {
                const currentAttractionName = pathData.attractionName;
                highlightAttractionGroup(svg, currentAttractionName);
            })
            .on('mouseleave', function(event, d) {
                resetAllConnections(svg);
            });
    };
    ...
    // 懸停效果：高亮指定景點組的所有連線
    const highlightAttractionGroup = (svg, attractionName) => {
        const targetGroup = svg.mainGroup || svg;
        ...
        // 指定景點組的連線高亮發光
        targetGroup.selectAll(`.attraction-connection[data-attraction-group="${attractionName}"]`)
            .transition()
            .duration(200)
            .style('fill-opacity', '0.6') // 更不透明
            .style('stroke-opacity', '1.0') // 完全不透明的邊框
            .style('stroke-width', '2px') // 加粗邊框
            .style('filter', 'drop-shadow(0 0 8px currentColor)'); // 發光效果
    };

    // 重置所有連線到初始狀態
    const resetAllConnections = (svg) => {
        const targetGroup = svg.mainGroup || svg;
        
        targetGroup.selectAll('.attraction-connection')
            .transition()
            .duration(300)
            .style('fill-opacity', '0.3') // 恢復原始透明度
            .style('stroke-opacity', '0.8') // 恢復原始透明度
            .style('stroke-width', '1px') // 恢復原始寬度
            .style('filter', 'none'); // 移除發光效果
    };
    ...
    return (
        <svg
            ...
            style={{
                ...
                width: '100vw',
                height: '100vh',
                pointerEvents: 'auto', // 改為 auto 以允許懸停事件
                zIndex: 1000,
                overflow: 'visible'
            }}
        />
