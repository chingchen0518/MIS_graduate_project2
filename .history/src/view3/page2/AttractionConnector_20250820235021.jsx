...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    const drawConnections = () => {
        ...
        svg
            ...
            .style('z-index', '1000')
            .style('overflow', 'hidden'); // 隱藏超出視窗的部分
    ...
    const isConnectionVisible = (from, to) => {
        // 使用 ScheduleContainer 的邊界而不是視窗邊界
        if (!containerRef?.current) return false;
        
        const containerRect = containerRef.current.getBoundingClientRect();
        const margin = 10; // 小的邊界緩衝區
        
        // 檢查起點是否在容器範圍內
        const fromVisible = from.left >= containerRect.left - margin && 
                           from.right <= containerRect.right + margin && 
                           from.top >= containerRect.top - margin && 
                           from.bottom <= containerRect.bottom + margin;
        
        // 檢查終點是否在容器範圍內
        const toVisible = to.left >= containerRect.left - margin && 
                         to.right <= containerRect.right + margin && 
                         to.top >= containerRect.top - margin && 
                         to.bottom <= containerRect.bottom + margin;
        
        // 只有當兩個端點都在容器範圍內時才繪製連線
        const bothVisible = fromVisible && toVisible;
        
        console.log(`👁️ 容器邊界檢查: ${from.name} -> ${to.name}`, {
            fromVisible,
            toVisible,
            bothVisible,
            container: { 
    ...
    const calculateConnectionPaths = (from, to) => {
        ...
        // 限制X坐標在視窗範圍內
        fromX = Math.max(0, Math.min(fromX, viewportWidth));
        toX = Math.max(0, Math.min(toX, viewportWidth));

        // 計算整個右側面的上下邊界，並限制在視窗範圍內
        let fromYTop = Math.max(0, from.top + padding);
        let fromYBottom = Math.min(viewportHeight, from.bottom - padding);
        let toYTop = Math.max(0, to.top + padding);
        let toYBottom = Math.min(viewportHeight, to.bottom - padding);
