  ...
  connection.query(validateQuery, [t_id, s_id, date], (validateErr, scheduleExists) => {
    ...
    connection.query(checkQuery, [u_id, s_id, t_id], (err, existing) => {
      ...
      if (existing.length > 0) {
        ...
        if (vote_type === null) {
          ...
          connection.query(deleteQuery, [u_id, s_id, t_id], (err, result) => {
            if (err) {
              ...
              console.log(`Vote cancelled for t_id:${t_id}, s_id:${s_id}, u_id:${u_id}, date:${date}`);
        ...
        // 如果vote_type為null且沒有現有投票，不需要做任何事
        if (vote_type === null) {
          console.log(`No vote to cancel for t_id:${t_id}, s_id:${s_id}, u_id:${u_id}, date:${date}`);
...
// 新增API：獲取特定用戶對特定行程的投票狀態
app.get('/api/user_vote/:t_id/:s_id/:u_id/:date', (req, res) => {
  const { t_id, s_id, u_id, date } = req.params;

  const query = `SELECT good, bad FROM Evaluate WHERE u_id = ? AND s_id = ? AND t_id = ?`;

  connection.query(query, [u_id, s_id, t_id], (err, results) => {
    if (err) {
      console.error('Error fetching user vote:', err);
      res.status(500).send('Failed to fetch user vote');
      ...
      console.log(`User vote for u_id:${u_id}, t_id:${t_id}, s_id:${s_id}, date:${date} = ${vote_type}`);
