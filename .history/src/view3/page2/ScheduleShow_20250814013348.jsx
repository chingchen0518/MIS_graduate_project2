...
const ScheduleShow = (props) => {
    ...
    const handleVote = async (voteType) => {
        try {
            // 為了測試多用戶投票，我們隨機使用不同的用戶ID (1-5)
            // 在實際應用中，這應該從用戶登錄狀態或context獲取
            const currentUserId = Math.floor(Math.random() * 5) + 1; // 隨機選擇1-5的用戶ID
            const formattedDate = formatDate(props.date);

            console.log(`User ${currentUserId} voting ${voteType} for schedule ${props.s_id}`);
            ...
            if (response.ok) {
                console.log(`Vote submitted successfully by user ${currentUserId}`);
                // 重新獲取投票數據
