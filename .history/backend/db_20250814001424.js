...
app.get('/api/schedule_budget/:t_id/:s_id', (req, res) => {
  ...
  const query = `SELECT SUM(a.budget) as total_budget 
                 ...
                 JOIN Schedule s ON si.s_id = s.s_id AND si.t_id = s.t_id
  ...
  connection.query(query, values, (err, results) => {
    if (err) {
      ...
      console.log(`Budget calculation for t_id:${t_id}, s_id:${s_id} = ${totalBudget}`);
...
// 新增API：計算特定日期的總預算
app.get('/api/schedule_budget_by_date/:t_id/:date', (req, res) => {
  ...
  const query = `SELECT SUM(a.budget) as total_budget 
                 ...
                 JOIN Attraction a ON si.a_id = a.a_id
                 JOIN Schedule s ON si.s_id = s.s_id AND si.t_id = s.t_id
                 WHERE si.t_id = ? AND s.date = ?`;
  const values = [t_id, date];

  connection.query(query, values, (err, results) => {
    if (err) {
      console.error('Error calculating budget by date:', err);
      res.status(200).json({ total_budget: 0 });
    } else {
      const totalBudget = results[0]?.total_budget || 0;
      console.log(`Budget calculation for t_id:${t_id}, date:${date} = ${totalBudget}`);
      res.status(200).json({ total_budget: totalBudget });
    }
  });
});
