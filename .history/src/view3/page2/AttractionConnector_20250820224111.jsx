...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    const calculateConnectionPaths = (from, to) => {
        ...
        const gapFactor = Math.max(0.6, Math.min(1.2, verticalDiff / 50)); // 根據高度差調整間距
        
        // 計算上下連接點的Y座標，增加偏移量讓線條更分散
        const offsetY = from.height * 0.35 * gapFactor; // 動態調整偏移
        
        // 上方連線的起點和終點 - 更靠近頂部
        const fromYTop = from.top + from.height * 0.2;
        const toYTop = to.top + to.height * 0.2;
        
        // 下方連線的起點和終點 - 更靠近底部  
        const fromYBottom = from.bottom - from.height * 0.2;
        const toYBottom = to.bottom - to.height * 0.2;
        
        // 計算弧形曲線的控制點
        const horizontalDistance = toX - fromX;
        const curvature = Math.max(horizontalDistance * 0.3, 30);
        ...
        return [
            {
                ...
                curvature,
                path: createCurvePath(fromX, fromYTop, toX, toYTop, curvature)
            },
            {
                type: 'bottom',
                fromX, fromY: fromYBottom, toX, toY: toYBottom,
                curvature,
                path: createCurvePath(fromX, fromYBottom, toX, toYBottom, curvature)
            }
        ];
    };

    const createCurvePath = (fromX, fromY, toX, toY, curvature) => {
        const path = d3.path();
        path.moveTo(fromX, fromY);
        path.bezierCurveTo(
            fromX + curvature, fromY,    // 第一個控制點
            toX - curvature, toY,        // 第二個控制點
            toX, toY                     // 終點
        );
