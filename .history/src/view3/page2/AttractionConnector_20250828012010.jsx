...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    const drawConnections = () => {
        ...
        allPaths.forEach((pathData, index) => {
            ...
            // 為每個連線添加唯一標識
            pathData.connectionId = `connection-${connectionIndex++}`;
            pathData.intersectionCount = hasIntersection ? 1 : 0;
    ...
    const drawConnectionPath = (svg, pathData, hasIntersection) => {
        ...
        const glowColor = hasIntersection ? '#FF8A80' : '#81C784';
        ...
        const connectionGroup = targetGroup.append('g')
            ...
            .attr('data-connection-id', pathData.connectionId || 'unknown')
            .attr('data-attraction', pathData.attractionName || 'unknown');
        
        // 繪製外層光暈（最大範圍）
        connectionGroup.append('path')
            .attr('d', pathData.path)
            .attr('class', 'attraction-connection-outer-glow')
            .style('fill', glowColor)
            .style('fill-opacity', hasIntersection ? '0.15' : '0.1')
            .style('stroke', 'none')
        ...
        const mainPath = connectionGroup.append('path')
            ...
            .style('fill-opacity', hasIntersection ? '0.5' : '0.4')
            .style('stroke', baseColor)
            .style('stroke-width', hasIntersection ? '3px' : '2px')
        ...
        // 為相交的連線添加特殊的脈動效果
        if (hasIntersection) {
            mainPath.style('animation', 'pulse-glow 2s ease-in-out infinite');
        }
        ...
        // 繪製中心引導線
        const centerLine = connectionGroup.append('path')
            ...
            .style('stroke-width', hasIntersection ? '3px' : '2px')
            .style('stroke-opacity', '0.9')
            .style('stroke-dasharray', hasIntersection ? '8,4' : '6,3')
            .style('stroke-linecap', 'round')
            .style('marker-end', 'url(#arrow)');
        
        // 為相交的連線使用不同的動畫速度
        if (hasIntersection) {
            centerLine.style('animation', 'arrow-flow 1.5s linear infinite');
        } else {
            centerLine.style('animation', 'arrow-flow 3s linear infinite');
        }
        
        // 添加起點標記
        const startPoint = connectionGroup.append('g')
            .attr('class', 'connection-start-point');
            
        startPoint.append('circle')
            .attr('cx', pathData.fromX)
            .attr('cy', centerY)
            .attr('r', hasIntersection ? 6 : 4)
        ...
        // 添加終點標記
        const endPoint = connectionGroup.append('g')
            .attr('class', 'connection-end-point');
        ...
        // 為終點添加方向指示器
        endPoint.append('polygon')
            .attr('points', `${pathData.toX-3},${centerYTo-2} ${pathData.toX+1},${centerYTo} ${pathData.toX-3},${centerYTo+2}`)
            .style('fill', '#ffffff')
            .style('opacity', '0.9');
        
        // 添加連線資訊（用於除錯和未來的互動功能）
        connectionGroup.append('title')
            .text(`連接: ${pathData.attractionName} (${hasIntersection ? '相交' : '獨立'})`);
    };
