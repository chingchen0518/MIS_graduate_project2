...
// API: 取得指定trip可選擇的景點類別（用於篩選）
app.get('/api/attraction_categories/:t_id', (req, res) => {
  const { t_id } = req.params;

  // 先獲取該trip的國家/地區信息
  const tripQuery = 'SELECT country FROM Trip WHERE t_id = ?';

  connection.query(tripQuery, [t_id], (tripErr, tripResults) => {
    if (tripErr) {
      console.error('❌ 取得trip信息時出錯：', tripErr.message);
      res.status(500).json({ error: tripErr.message });
      return;
    }
    ...
    const country = tripResults[0].country;

    // 根據trip的國家獲取相關景點類別，如果沒有特定國家則返回所有類別
    const query = `
    ...
    connection.query(query, (err, results) => {
      if (err) {
        console.error('❌ 取得景點類別時出錯：', err.message);
        res.status(500).json({ error: err.message });
        return;
      }

      console.log(`✅ 成功取得trip ${t_id} (${country}) 的景點類別：`, results);
      res.json({
        success: true,
...
app.get('/api/view3_trip_budget_range/:t_id', (req, res) => {
  ...
  const query = `
    SELECT MIN(a.budget) as min_budget, MAX(a.budget) as max_budget
    FROM attraction a
    INNER JOIN schedule_include si ON a.a_id = si.a_id
    INNER JOIN schedule s ON si.s_id = s.s_id
    WHERE s.t_id = ? AND a.budget IS NOT NULL
  `;

  connection.query(query, [t_id], (err, results) => {
    if (err) {
      console.error('❌ 取得trip預算範圍時出錯：', err.message);
      res.status(500).json({ error: err.message });
      return;
    }

    console.log('✅ 成功取得trip預算範圍：', results);
    const budgetRange = results[0];
    res.json({
      success: true,
      minBudget: budgetRange.min_budget || 0,
      maxBudget: budgetRange.max_budget || 1000
    });
  });
});
