...
const ScheduleShow = (props) => {
    ...
    useEffect(() => {
        const fetchVotes = async () => {
            try {
                ...
                const userVoteResponse = await fetch(`http://localhost:3001/api/schedule_votes/${props.t_id}/${props.s_id}/${currentUserId}`);
    ...
    }, [props.t_id, props.s_id, props.date]); // 移除 formatDate 依賴項，因為它已經被 useCallback 包裝且沒有依賴項
    ...
    }, [currentUserVote, props.t_id, props.s_id, props.date]); // 移除 formatDate 依賴項，因為它已經被 useCallback 包裝且沒有依賴項
                    ...
                    <div className="vote_bar_container_small">
                        <div
                            ...
                            style={{
                                width: voteData.total_likes + voteData.total_dislikes === 0
                                    ? '50%'
                                    : `${Math.max(5, Math.min(95, (voteData.total_likes / (voteData.total_likes + voteData.total_dislikes)) * 100))}%`,
                        ...
                        <div
                            ...
                            style={{
                                width: voteData.total_likes + voteData.total_dislikes === 0
                                    ? '50%'
                                    : `${Math.max(5, Math.min(95, (voteData.total_dislikes / (voteData.total_likes + voteData.total_dislikes)) * 100))}%`,
