...
const AttractionConnector = ({ schedules = [], containerRef, timeColumnWidth = 0 }) => {
    ...
    const drawConnections = () => {
        ...
        svg
            ...
            .style('pointer-events', 'auto') // 改為 auto 以啟用鼠標事件
            .style('z-index', '1000')
                ...
                if (next.scheduleIndex - current.scheduleIndex === 1) {
                    ...
                    if (isConnectionVisible(current, next)) {
                        ...
                        paths.forEach(path => {
                            ...
                            path.fromScheduleId = current.scheduleId; // 添加來源行程ID
                            path.toScheduleId = next.scheduleId; // 添加目標行程ID
        ...
        allPaths.forEach((pathData, index) => {
            ...
            const connectionInfo = {
                fromSchedule: `行程${pathData.fromScheduleId}`,
                toSchedule: `行程${pathData.toScheduleId}`,
                fromScheduleId: pathData.fromScheduleId,
                toScheduleId: pathData.toScheduleId
            };

            drawConnectionPath(svg, pathData, hasIntersection, connectionInfo);
        });
    ...
    const drawConnectionPath = (svg, pathData, hasIntersection, connectionInfo) => {
        const color = hasIntersection ? '#FF4444' : '#2196F3'; // 紅色如果相交，否則藍色
        const isHovered = hoveredConnection && 
            hoveredConnection.attractionName === pathData.attractionName;

        // 使用主要群組繪製，自動應用裁剪
        const targetGroup = svg.mainGroup || svg;

        const pathElement = targetGroup.append('path')
            .attr('d', pathData.path)
            .attr('class', 'attraction-connection')
            .attr('data-attraction', pathData.attractionName)
            .style('fill', color) // 使用填充而不是描邊
            .style('fill-opacity', isHovered ? '0.6' : '0.3') // 懸停時增加透明度
            .style('stroke', color) // 邊框同色
            .style('stroke-width', isHovered ? '2px' : '1px') // 懸停時加粗邊框
            .style('stroke-opacity', isHovered ? '1.0' : '0.8') // 懸停時完全不透明
            .style('cursor', 'pointer')
            .style('transition', 'all 0.3s ease')
            .style('pointer-events', 'all'); // 啟用鼠標事件
        ...
        pathElement
            .on('mouseenter', function(event) {
                ...
                d3.select(this)
                    ...
                    .style('filter', 'drop-shadow(0 0 8px currentColor) brightness(1.2)');
                
                // 淡化其他線條
                svg.selectAll('.attraction-connection')
                    .filter(function() { return this !== event.target; })
                    .style('opacity', '0.3');
                
                setHoveredConnection({ attractionName: pathData.attractionName });
                ...
                setTooltip({
                    ...
                    x: mousePos[0] + 10,
                    y: mousePos[1] - 10,
                    content: `🏖️ ${pathData.attractionName}\n\n📋 行程比較:\n${connectionInfo.fromSchedule} ↔ ${connectionInfo.toSchedule}\n\n${hasIntersection ? '⚠️ 時段衝突' : '✅ 時段相容'}\n\n💡 點擊查看詳細資訊`
                });
            })
            ...
            .on('click', function(event) {
                ...
                const scheduleDetails = schedules
                    .filter(s => [connectionInfo.fromScheduleId, connectionInfo.toScheduleId].includes(s.s_id))
                    .map(s => `• 行程 ${s.s_id}: ${s.title || s.date}`)
                    .join('\n');
                
                const conflictInfo = hasIntersection 
                    ? '\n⚠️ 時段衝突詳情:\n這兩個行程在此景點的時間安排有重疊，可能造成擁擠或時間衝突。'
                    : '\n✅ 時段相容:\n這兩個行程在此景點的時間安排沒有衝突，可以順利進行。';
                
                alert(`🏖️ 景點連接詳情\n\n📍 景點名稱: ${pathData.attractionName}\n\n📋 相關行程:\n${scheduleDetails}${conflictInfo}\n\n� 提示: 您可以拖曳行程卡片重新排列位置\n\n點擊確定關閉詳情`);
            });
    };
        ...
        <>
            <svg
                ...
                style={{
                    ...
                    width: '100vw',
                    height: '100vh',
                    pointerEvents: 'auto', // 改為 auto 以啟用鼠標事件
                    zIndex: 1000,
                    overflow: 'visible'
                }}
            />
