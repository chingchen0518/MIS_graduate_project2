...
app.post('/api/schedule_vote/:t_id/:s_id/:u_id', (req, res) => {
  ...
  connection.query(checkQuery, [u_id, s_id, t_id], (err, existing) => {
    ...
    if (existing.length > 0) {
      ...
      const good = vote_type === 'like' ? 1 : 0;
      const bad = vote_type === 'dislike' ? 1 : 0;
      
      connection.query(updateQuery, [good, bad, u_id, s_id, t_id], (err, result) => {
        if (err) {
      ...
      connection.query(insertQuery, [u_id, s_id, t_id, good, bad], (err, result) => {
        if (err) {
