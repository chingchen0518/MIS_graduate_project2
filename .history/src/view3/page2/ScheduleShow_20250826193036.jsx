...
const ScheduleShow = (props) => {
    ...
    const [tripMembers, setTripMembers] = useState([]); // 新增：行程成員數據
    const [currentUserId, setCurrentUserId] = useState(1); // 當前用戶ID，實際應該從context獲取
        ...
        const fetchTripMembers = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    console.error('Trip members API failed');
                }
            } catch (error) {
    ...
    // 生成光圈虛線的函數
    const generateAuraLines = (totalMembers, activeCount, isLike = true) => {
        const lines = [];
        const radius = 30; // 光圈半徑
        const centerX = 25; // 按鈕中心X座標
        const centerY = 25; // 按鈕中心Y座標
        
        for (let i = 0; i < totalMembers; i++) {
            // 等差分布角度，從12點鐘方向開始 (90度開始，順時針)
            const angle = (90 - (360 / totalMembers) * i) * (Math.PI / 180);
                ...
                <div className="user_info">
                    ...
                    <div className="vote_buttons">
                        <div className="vote_button_container">
                            ...
                            <svg className="vote_aura" width="60" height="60" style={{ position: 'absolute', top: '-5px', left: '-5px', pointerEvents: 'none' }}>
                                {generateAuraLines(tripMembers.length || 5, voteData.total_likes, true)}
                            </svg>
                        </div>
                        ...
                        <div className="vote_button_container">
                            ...
                            <svg className="vote_aura" width="60" height="60" style={{ position: 'absolute', top: '-5px', left: '-5px', pointerEvents: 'none' }}>
                                {generateAuraLines(tripMembers.length || 5, voteData.total_dislikes, false)}
                            </svg>
                        </div>
                    </div>
                </div>
