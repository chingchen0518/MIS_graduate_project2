        ...
        schedules.forEach((schedule, scheduleIndex) => {
            ...
            attractionElements.forEach((attractionEl, index) => {
                ...
                console.log(`景點 ${index + 1}: ${attractionName}`, {
                    viewport: { top: attractionRect.top, left: attractionRect.left },
                    container: { top: currentContainerRect.top, left: currentContainerRect.left },
                    relative: { 
                        top: attractionRect.top - currentContainerRect.top, 
                        left: attractionRect.left - currentContainerRect.left 
                    }
                });

                if (attractionName && attractionRect.width > 0 && attractionRect.height > 0) {
                    // 計算相對於容器的位置
                    const relativeTop = attractionRect.top - currentContainerRect.top;
                    const relativeLeft = attractionRect.left - currentContainerRect.left;
                    
                    attractionPositions.push({
                        name: attractionName,
                        scheduleIndex: scheduleIndex,
                        scheduleId: schedule.s_id,
                        x: relativeLeft + attractionRect.width / 2,
                        y: relativeTop + attractionRect.height / 2,
                        left: relativeLeft,
                        right: relativeLeft + attractionRect.width,
                        top: relativeTop,
                        bottom: relativeTop + attractionRect.height,
