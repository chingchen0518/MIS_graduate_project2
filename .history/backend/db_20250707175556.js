...
// Business（複合主鍵 a_id, w_day, period）
app.get('/api/business', (req, res) => {
  const { a_id, w_day, period } = req.query;
  connection.query(
    'SELECT * FROM Business WHERE a_id = ? AND w_day = ? AND period = ?',
    [a_id, w_day, period],
    (err, results) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(results);
...
// Evaluate（複合鍵 u_id, s_id）
app.get('/api/evaluate', (req, res) => {
  const { u_id, s_id } = req.query;
  connection.query(
    'SELECT * FROM Evaluate WHERE u_id = ? AND s_id = ?',
    [u_id, s_id],
    (err, results) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(results);
...
// Include2（複合主鍵 s_id, a_id）
app.get('/api/include2', (req, res) => {
  const { s_id, a_id } = req.query;
  connection.query(
    'SELECT * FROM Include2 WHERE s_id = ? AND a_id = ?',
    [s_id, a_id],
    (err, results) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(results);
