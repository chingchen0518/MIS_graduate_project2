...
const Filter = ({ t_id, onCategoryChange, onFilterChange }) => {
    ...
    // 當篩選條件改變時，通知父組件
    useEffect(() => {
        // 只有當 users 已經加載完成且 selectedUsers 不為空時，才通知父組件
        if (onFilterChange && users.length > 0) {
            console.log('🔄 Filter change triggered:', { costRange, selectedAttractions, selectedUsers });
            onFilterChange(costRange, selectedAttractions, selectedUsers);
        }
    }, [costRange, selectedAttractions, selectedUsers, onFilterChange, users.length]);
    ...
    const handleUserToggle = (userId) => {
        console.log('👤 User toggle clicked:', userId);
        setSelectedUsers(prev => {
            const newSelectedUsers = prev.includes(userId) 
                ? prev.filter(id => id !== userId)
                : [...prev, userId];
            console.log('👤 Updated selectedUsers:', newSelectedUsers);
