...
app.get('/api/schedule_budget/:t_id/:s_id', (req, res) => {
  ...
  const query = `SELECT SUM(a.budget) as total_budget FROM Schedule_include s
                   JOIN Attraction a ON s.a_id = a.a_id
                   WHERE s.t_id = ? AND s.s_id = ?`;
  const values = [t_id, s_id];
  ...
  connection.query(query, values, (err, results) => {
    if (err) {
      ...
      res.status(500).send('Failed to calculate budget');
    } else {
...
app.get('/api/schedule_votes/:t_id/:s_id', (req, res) => {
  ...
  // 獲取該行程的所有投票統計
  const query = `SELECT 
                   SUM(onelove) as total_likes,
                   SUM(twolove) as total_dislikes
                 FROM Support s
                 JOIN schedule_include si ON s.a_id = si.a_id AND s.t_id = si.t_id
...
app.post('/api/schedule_vote/:t_id/:s_id/:u_id', (req, res) => {
  ...
  // 首先獲取該行程的所有景點
  const getAttractionsQuery = `SELECT DISTINCT a_id FROM schedule_include WHERE t_id = ? AND s_id = ?`;

  connection.query(getAttractionsQuery, [t_id, s_id], (err, attractions) => {
    if (err) {
    ...
    if (attractions.length === 0) {
      res.status(404).send('No attractions found for this schedule');
      return;
    }
    ...
    const promises = attractions.map(attraction => {
      return new Promise((resolve, reject) => {
        const checkQuery = `SELECT * FROM support WHERE u_id = ? AND a_id = ? AND t_id = ?`;

        connection.query(checkQuery, [u_id, attraction.a_id, t_id], (err, existing) => {
          if (err) {
            reject(err);
            return;
          }
          ...
          if (existing.length > 0) {
            ...
            const updateQuery = vote_type === 'like'
              ? `UPDATE support SET onelove = onelove + 1 WHERE u_id = ? AND a_id = ? AND t_id = ?`
              : `UPDATE support SET twolove = twolove + 1 WHERE u_id = ? AND a_id = ? AND t_id = ?`;

            connection.query(updateQuery, [u_id, attraction.a_id, t_id], (err, result) => {
              if (err) reject(err);
              else resolve(result);
            });
          } else {
            // 插入新投票
            const insertQuery = `INSERT INTO support (u_id, a_id, t_id, onelove, twolove) VALUES (?, ?, ?, ?, ?)`;
            const values = vote_type === 'like'
              ? [u_id, attraction.a_id, t_id, 1, 0]
              : [u_id, attraction.a_id, t_id, 0, 1];
