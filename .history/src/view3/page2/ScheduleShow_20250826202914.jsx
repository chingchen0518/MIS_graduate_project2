...
const ScheduleShow = (props) => {
    ...
    useEffect(() => {
        const fetchVotes = async () => {
            try {
                ...
                console.log('User vote response status:', userVoteResponse.status);
                if (userVoteResponse.ok) {
                    ...
                    console.error('User vote API failed with status:', userVoteResponse.status);
                    const errorText = await userVoteResponse.text();
                    console.error('User vote API error:', errorText);
    ...
    const handleVote = async (voteType) => {
        try {
            ...
            console.log(`Current user vote: ${currentUserVote}, clicking: ${voteType}`);
            
            // 如果點擊的是已經投過的票，則取消投票
            const finalVoteType = currentUserVote === voteType ? null : voteType;
            
            console.log(`Sending vote: ${finalVoteType}`);
