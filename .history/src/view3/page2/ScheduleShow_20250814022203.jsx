...
const ScheduleShow = (props) => {
    ...
    // 檢查使用者是否被選中（這裡使用 props.u_id 作為該 Schedule 的創建者）
    // 當 selectedUsers 為空陣列時表示還未初始化，應該顯示所有
    // 當 selectedUsers 有內容時，檢查當前使用者是否在選中列表中
    // 注意：確保類型匹配（都轉換為數字進行比較）
    const isUserSelected = selectedUsers.length === 0 || 
        selectedUsers.some(id => Number(id) === Number(props.u_id));

    // 計算透明度
    const getScheduleOpacity = () => {
        if (!isBudgetInRange) return 0.3; // 預算不在範圍內時變透明
        if (!isUserSelected) return 0.3; // 使用者未被選中時變透明
        return 1;
    };
    ...
    // 添加調試信息
    useEffect(() => {
        console.log('📅 ScheduleShow Debug:', {
            'Schedule u_id': props.u_id, 
            'Type of u_id': typeof props.u_id,
            'selectedUsers': selectedUsers,
            'selectedUsers types': selectedUsers.map(id => typeof id),
            'isUserSelected': isUserSelected,
            'schedule title': props.title
        });
    }, [props.u_id, selectedUsers, isUserSelected, props.title]);
